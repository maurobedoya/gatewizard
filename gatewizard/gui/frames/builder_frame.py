...
class BuilderFrame(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        self.workfile_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.workfile_frame = ctk.CTkFrame(
            self.workfile_section, fg_color="transparent"
        )
        ...
        self.lipids_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.water_label = ctk.CTkLabel(
            self.ff_row1, text="Water Model:", font=FONTS["body"]
        )
        ...
        self.protein_ff_label = ctk.CTkLabel(
            self.ff_row1, text="Protein FF:", font=FONTS["body"]
        )
        ...
        self.lipid_ff_label = ctk.CTkLabel(
            self.ff_row1, text="Lipid FF:", font=FONTS["body"]
        )
        ...
        self.salt_conc_label = ctk.CTkLabel(
            self.salt_frame, text="Concentration (M):", font=FONTS["body"]
        )

        self.salt_conc_entry = ctk.CTkEntry(
            self.salt_frame, width=80, height=WIDGET_SIZES["entry_height"]
        )
        self.salt_conc_entry.insert(0, "0.15")

        self.cation_label = ctk.CTkLabel(
            self.salt_frame, text="Cation:", font=FONTS["body"]
        )
        ...
        self.anion_label = ctk.CTkLabel(
            self.salt_frame, text="Anion:", font=FONTS["body"]
        )
        ...
        self.water_distance_frame = ctk.CTkFrame(
        ...
        self.water_distance_entry = ctk.CTkEntry(
            self.water_distance_frame, width=80, height=WIDGET_SIZES["entry_height"]
        )
        ...
        self.protonation_help_text = ctk.CTkTextbox(
            self.protonation_frame, height=60, font=FONTS["small"]
        )
...
4. Salt: ion addition (cation/anion and concentration) is applied during parametrization when enabled"""
        ...
        self.progress_tracker = ProgressTracker(
            self.progress_section, working_directory=self.working_directory
        )
    ...
    def _update_propka_help_text(self):
        ...
        if not hasattr(self, "propka_stage_var") or not hasattr(
            self, "propka_help_text"
        ):
            return
    ...
    def _setup_layout(self):
        ...
        # Working directory section
        self.workdir_section.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )

        self.workdir_label.pack(
            anchor="w",
            padx=LAYOUT["padding_medium"],
            pady=(LAYOUT["padding_medium"], LAYOUT["padding_small"]),
        )

        self.workdir_frame.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )
        self.workdir_entry.pack(
            side="left", fill="x", expand=True, padx=(0, LAYOUT["padding_small"])
        )
        self.workdir_browse_button.pack(side="right", padx=LAYOUT["padding_small"])

        # Working file section
        self.workfile_section.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )

        self.workfile_label.pack(
            anchor="w",
            padx=LAYOUT["padding_medium"],
            pady=(LAYOUT["padding_medium"], LAYOUT["padding_small"]),
        )

        self.workfile_frame.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )
        self.workfile_entry.pack(
            side="left", fill="x", expand=True, padx=(0, LAYOUT["padding_small"])
        )
        self.workfile_browse_button.pack(side="right", padx=LAYOUT["padding_small"])

        # Output folder name section
        self.outputname_section.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )

        self.outputname_label.pack(
            anchor="w",
            padx=LAYOUT["padding_medium"],
            pady=(LAYOUT["padding_medium"], LAYOUT["padding_small"]),
        )

        self.outputname_frame.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )
        self.outputname_entry.pack(
            side="left", fill="x", expand=True, padx=LAYOUT["padding_small"]
        )

        # Ligand parametrization section
        self.ligand_param_widget.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )

        # Lipids section
        self.lipids_section.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )

        self.lipids_label.pack(
            anchor="w",
            padx=LAYOUT["padding_medium"],
            pady=(LAYOUT["padding_medium"], LAYOUT["padding_small"]),
        )

        self.leaflets_frame.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )
        self.upper_leaflet.pack(
            side="left", fill="both", expand=True, padx=(0, LAYOUT["padding_small"])
        )
        self.lower_leaflet.pack(
            side="left", fill="both", expand=True, padx=(LAYOUT["padding_small"], 0)
        )

        # Force field section
        self.ff_section.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_medium"]
        )
        ...
        # Salt options
        self.salt_frame.pack(
            fill="x", padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )

        self.salt_conc_label.pack(
            side="left", padx=(LAYOUT["padding_large"], LAYOUT["padding_small"])
        )
        self.salt_conc_entry.pack(side="left", padx=LAYOUT["padding_small"])

        self.cation_label.pack(
            side="left", padx=(LAYOUT["padding_medium"], LAYOUT["padding_small"])
        )
        self.cation_combo.pack(side="left", padx=LAYOUT["padding_small"])

        self.anion_label.pack(
            side="left", padx=(LAYOUT["padding_medium"], LAYOUT["padding_small"])
        )
    ...
    def _load_defaults(self):
        ...
        try:
            ...
            logger.info(
                f"Defaults loaded - Water: {recommendations['water_model']}, Protein: {recommendations['protein_ff']}, Lipid: {recommendations['lipid_ff']}, Default lipid: POPC"
            )
    ...
    def _collect_inputs(self) -> dict:
        ...
        upper_ratio_str = (
            ":".join([str(ratio) for _, ratio in upper_lipids_ratios])
            if upper_lipids_ratios
            else ""
        )
        lower_ratio_str = (
            ":".join([str(ratio) for _, ratio in lower_lipids_ratios])
            if lower_lipids_ratios
            else ""
        )
        ...
        try:
            ...
            if not inputs["pdb_file"]:
                messagebox.showerror(
                    "Validation Failed", "Please select a working PDB file"
                )
            ...
            if not os.path.exists(inputs["pdb_file"]):
                messagebox.showerror(
                    "Validation Failed", f"PDB file not found: {inputs['pdb_file']}"
                )
                self.prepare_button.configure(state="disabled")
                return
            ...
            if not inputs["upper_lipids"] and not inputs["lower_lipids"]:
                messagebox.showerror(
                    "Validation Failed", "Please select lipids for at least one leaflet"
                )
                self.prepare_button.configure(state="disabled")
                return
            ...
            if valid:
                ...
                if "WARNING" in msg:
                    ...
                    messagebox.showinfo(
                        ...
                        "All inputs are valid. Ready to start preparation.",
                    )
                    if self.status_callback:
            ...
            messagebox.showerror(
                "Validation Error", f"Error during validation: {str(e)}"
            )
            self.prepare_button.configure(state="disabled")
    ...
    def _start_preparation(self):
        ...
        try:
            ...
            if not working_dir:
                messagebox.showerror(
                    "No Directory", "Please select a working directory."
                )
                return
            ...
            config = {
                ...
                "salt_concentration": (
                    float(inputs["salt_concentration"])
                    if inputs["salt_concentration"]
                    else 0.15
                ),
    ...
    def _validate_stage2_pdb(self, packed_pdb_path: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if found_modifications:
                message = (
                    f"Found Propka modifications: {', '.join(found_modifications)}"
                )
                return True, message
            else:
...
"""
        ...
        help_label = ctk.CTkLabel(
            help_frame, text="Propka Two-Stage Workflow", font=FONTS["heading"]
        )
        help_label.pack(pady=(0, 10))

        help_textbox = ctk.CTkTextbox(help_frame, font=FONTS["small"], wrap="word")
        help_textbox.pack(fill="both", expand=True, pady=(0, 10))
        help_textbox.insert("1.0", help_text)
        help_textbox.configure(state="disabled")

        close_button = ctk.CTkButton(
            help_frame, text="Close", command=help_window.destroy
        )
        close_button.pack()
    ...
    def _open_propka_workflow_helper(self):
        ...
        try:
            from gatewizard.gui.widgets.propka_workflow_helper import (
                PropkaWorkflowDialog,
            )
            ...
            messagebox.showerror(
                "Error", f"Could not open Propka workflow helper: {str(e)}"
            )
    ...
    def update_fonts(self, scaled_fonts):
        ...
        try:
            ...
            for btn in [
                "workdir_browse_button",
                "workfile_browse_button",
                "prepare_button",
                "validate_button",
                "defaults_button",
                "refresh_button",
                ...
                if hasattr(self, btn):
                    safe_update_font(
                        getattr(self, btn), scaled_fonts["body"], width=120, height=32
                    )
            ...
            if hasattr(self, "upper_leaflet") and hasattr(
                self.upper_leaflet, "update_fonts"
            ):
                self.upper_leaflet.update_fonts(scaled_fonts)
            if hasattr(self, "lower_leaflet") and hasattr(
                self.lower_leaflet, "update_fonts"
            ):
                self.lower_leaflet.update_fonts(scaled_fonts)

            # Update ligand param widget
            if hasattr(self, "ligand_param_widget") and hasattr(
                self.ligand_param_widget, "update_fonts"
            ):
