...
class PropkaWorkflowDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        self.intro_frame = ctk.CTkFrame(
            self.main_frame, fg_color=COLOR_SCHEME["content_inside_bg"]
        )

        self.intro_text = ctk.CTkTextbox(
            self.intro_frame, height=80, font=FONTS["small"], wrap="word"
        )
        ...
        self.step1_label = ctk.CTkLabel(
            self.step1_frame, text="Step 1: Select Files", font=FONTS["heading"]
        )
        ...
        self.original_pdb_input_frame = ctk.CTkFrame(
            self.original_pdb_frame, fg_color="transparent"
        )
        ...
        self.packed_pdb_input_frame = ctk.CTkFrame(
            self.packed_pdb_frame, fg_color="transparent"
        )
        ...
        self.step2_frame = ctk.CTkFrame(
            self.main_frame, fg_color=COLOR_SCHEME["content_inside_bg"]
        )

        self.step2_label = ctk.CTkLabel(
            self.step2_frame, text="Step 2: Propka Analysis", font=FONTS["heading"]
        )

        self.propka_params_frame = ctk.CTkFrame(
            self.step2_frame, fg_color="transparent"
        )

        self.ph_label = ctk.CTkLabel(
            self.propka_params_frame, text="Target pH:", font=FONTS["body"]
        )

        self.ph_entry = ctk.CTkEntry(self.propka_params_frame, width=80)
        self.ph_entry.insert(0, "7.0")

        self.version_label = ctk.CTkLabel(
            self.propka_params_frame, text="Propka Version:", font=FONTS["body"]
        )

        self.version_combo = ctk.CTkComboBox(
            self.propka_params_frame, values=["3"], width=80
        )
        ...
        self.results_label = ctk.CTkLabel(
            self.results_frame, text="Propka Results:", font=FONTS["body"]
        )

        self.results_text = ctk.CTkTextbox(
            self.results_frame, height=200, font=FONTS["small"]
        )

        # Step 3: Modify residue names
        self.step3_frame = ctk.CTkFrame(
            self.main_frame, fg_color=COLOR_SCHEME["content_inside_bg"]
        )

        self.step3_label = ctk.CTkLabel(
            self.step3_frame, text="Step 3: Modify Residue Names", font=FONTS["heading"]
        )
        ...
        self.output_label = ctk.CTkLabel(
            self.output_frame, text="Modified PDB output file:", font=FONTS["body"]
        )

        self.output_input_frame = ctk.CTkFrame(
            self.output_frame, fg_color="transparent"
        )

        self.output_entry = ctk.CTkEntry(
            self.output_input_frame, placeholder_text="Output file path...", width=400
        )
        ...
        self.summary_label = ctk.CTkLabel(
            self.summary_frame, text="Modifications Summary:", font=FONTS["body"]
        )

        self.summary_text = ctk.CTkTextbox(
            self.summary_frame, height=100, font=FONTS["small"]
        )

        # Step 4: Ready for Stage 2
        self.step4_frame = ctk.CTkFrame(
            self.main_frame, fg_color=COLOR_SCHEME["content_inside_bg"]
        )

        self.step4_label = ctk.CTkLabel(
            self.step4_frame, text="Step 4: Ready for Stage 2", font=FONTS["heading"]
        )
        ...
        self.copy_path_button = ctk.CTkButton(
            self.step4_frame, text="Copy Output Path", command=self._copy_output_path
        )
        ...
        self.close_button = ctk.CTkButton(
            self.buttons_frame, text="Close", command=self._close_dialog
        )

        self.help_button = ctk.CTkButton(
            self.buttons_frame, text="Help", command=self._show_help
        )

        self.reset_button = ctk.CTkButton(
            self.buttons_frame, text="Reset", command=self._reset_dialog
        )
    ...
    def _display_propka_results(self, results: List[Dict], ph: float):
        ...
        messagebox.showinfo(
            "Success",
            f"Propka analysis completed! Found {modifications_count} suggested modifications.",
        )
    ...
    def _auto_apply_modifications(self):
        ...
        if not output_file:
            ...
            output_file = str(
                base_path.with_name(f"{base_path.stem}_propka_modified.pdb")
            )
            self.output_entry.delete(0, "end")
        ...
        try:
            ...
            if success:
                ...
                messagebox.showerror(
                    "Error", f"Failed to apply modifications: {message}"
                )
    ...
    def _manual_modifications(self):
        ...
        if not self.propka_results:
            messagebox.showwarning(
                "Warning", "Run Propka analysis first to get suggested modifications."
            )
        ...
        if hasattr(dialog, "modifications") and dialog.modifications:
            ...
            self._update_summary(
                self.custom_modifications,
                success=False,
                message="Manual modifications defined",
            )
    ...
    def _preview_modifications(self):
        ...
        if not self.propka_results and not self.custom_modifications:
            messagebox.showinfo(
                "No Modifications",
                "No modifications to preview. Run Propka analysis or define manual modifications first.",
            )
            return
        ...
        if self.custom_modifications:
            ...
            for residue in self.propka_results:
                ...
                if default_state != residue["residue"]:
                    chain_info = (
                        f" (Chain {residue['chain']})" if residue.get("chain") else ""
                    )
        ...
        close_btn = ctk.CTkButton(
            preview_dialog, text="Close", command=preview_dialog.destroy
        )
        close_btn.pack(pady=(0, 20))

    def _update_summary(
        self, modifications: Dict[str, str], success: bool, message: str
    ):
...
class ManualModificationsDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        self.modifications_frame = ctk.CTkScrollableFrame(
            self.main_frame, height=300, fg_color=COLOR_SCHEME["background"]
        )
        ...
        self.add_button = ctk.CTkButton(
            self.add_frame, text="Add Modification", command=self._add_modification_row
        )

        self.clear_button = ctk.CTkButton(
            self.add_frame, text="Clear All", command=self._clear_all_modifications
        )
        ...
        self.ok_button = ctk.CTkButton(
            self.buttons_frame, text="Apply", command=self._apply_modifications
        )
    ...
    def _populate_suggestions(self):
        ...
        for residue in self.propka_results:
            default_state = preparation_manager.get_default_protonation_state(
                residue, self.current_ph
            )
            ...
            if default_state != residue["residue"]:
                chain = residue.get("chain", "")
                res_display = (
                    f"{residue['res_id']} (Chain {chain})"
                    if chain
                    else str(residue["res_id"])
                )
                key = (
                    f"{chain}:{residue['res_id']}" if chain else str(residue["res_id"])
                )
    ...
    def _add_modification_row(
        self, residue_id: str = "", current_name: str = "", suggested_name: str = ""
    ):
        """Add a row for defining a modification."""
        row_frame = ctk.CTkFrame(
            self.modifications_frame, fg_color=COLOR_SCHEME["canvas"]
        )
        ...
        arrow_label = ctk.CTkLabel(
            row_frame, text="", image=self._right_arrow, width=20
        )
...
def create_residue_mapping_from_propka(
    propka_results: List[Dict], ph: float
) -> Dict[str, str]:
    """
...
def get_modification_summary(modifications: Dict[str, str]) -> str:
    ...
    summary_lines = [
        f"Residue {res_id} → {new_name}" for res_id, new_name in modifications.items()
    ]
