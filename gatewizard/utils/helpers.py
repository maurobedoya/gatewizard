...
def create_directory_robust(
    directory_path: Union[str, Path], max_retries: int = 3
) -> None:
    """
        ...
        try:
            ...
            if directory_path.exists() and directory_path.is_dir():
                ...
                try:
                    ...
                    logger.warning(
                        f"Directory exists but not writable: {directory_path}"
                    )
            ...
            logger.warning(
                f"FileExistsError on attempt {attempt + 1}: {directory_path}"
            )
            ...
            if directory_path.exists() and not directory_path.is_dir():
                ...
                try:
                    ...
                    try:
                        backup_path = directory_path.with_name(
                            f"{directory_path.name}.backup.{int(time.time())}"
                        )
                        ...
                        if attempt >= max_retries:
                            raise OSError(
                                f"Cannot remove or move conflicting file: {directory_path}"
                            )
            ...
            if "File exists" in str(e) or e.errno == 17:  # EEXIST
                ...
                if directory_path.exists():
                    if directory_path.is_dir():
                        logger.info(
                            f"Directory already exists and is valid: {directory_path}"
                        )
                        return  # Success - directory exists
                    else:
                        # It's a file, remove it
                        logger.info(
                            f"Removing file at directory path: {directory_path}"
                        )
                        try:
                            directory_path.unlink()
                        except OSError:
