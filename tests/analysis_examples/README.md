...
Examples 14 and 15 use `equilibration_folder/system.pdb` and the first three equilibration DCD trajectories. Lipid headgroups are selected with `resname PC and name P31` (AMBER POPC).

#### Example 14: Area per Lipid

`analysis_example_14.py` — Voronoi area-per-lipid analysis, plot, and JSON API.
...
`analysis_example_15.py` — interleaflet membrane thickness, plot, and JSON API.
...
```bash
python analysis_example_14.py
python analysis_example_15.py
```
