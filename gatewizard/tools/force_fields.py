...
class ForceFieldManager:
    ...
    def validate_combination(
        ...
        if protein_ff not in water_info.compatible_with:
            warnings.append(
                f"Water model {water_model} and protein force field {protein_ff}"
            )

        # Check water-lipid compatibility
        if lipid_ff not in water_info.compatible_with:
            warnings.append(
                f"Water model {water_model} and lipid force field {lipid_ff}"
            )

        # Check protein-lipid compatibility
        if lipid_ff not in protein_info.compatible_with:
            warnings.append(
                f"Protein force field {protein_ff} and lipid force field {lipid_ff}"
            )
    ...
    def get_force_field_info(
        self, ff_name: str, ff_type: str
    ) -> Optional[ForceFieldInfo]:
        """
