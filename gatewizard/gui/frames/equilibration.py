...
class _SelectionEditorDialog(ctk.CTkToplevel):
    ...
    def __init__(
        self, parent, title: str, initial_selection: str, pdb_path: Optional[str] = None
    ):
...
class EquilibrationFrame(ctk.CTkFrame):
    ...
    def __init__(
        ...
        self.mda_selections: Dict[str, Dict[str, str]] = (
            {}
        )  # stage -> {constraint: mda_sel}
    ...
    def _create_title_section(self):
        ...
        self.title_frame = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
    ...
    def _create_workdir_section(self):
        ...
        self.workdir_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.workdir_browse_btn = ctk.CTkButton(
            self.workdir_frame, text="Browse", width=80, command=self._browse_workdir
        )

    def _create_inputfolder_section(self):
        """Create the input folder section for prepared system files."""
        self.inputfolder_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.inputfolder_frame = ctk.CTkFrame(
            self.inputfolder_section, fg_color="transparent"
        )
    ...
    def _create_outputname_section(self):
        ...
        self.outputname_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.outputname_frame = ctk.CTkFrame(
            self.outputname_section, fg_color="transparent"
        )
    ...
    def _create_engine_section(self):
        ...
        self.engine_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.engine_settings_frame = ctk.CTkFrame(
            self.engine_section, fg_color="transparent"
        )
    ...
    def _create_protocol_section(self):
        ...
        self.charmm_gui_scheme_frame = ctk.CTkFrame(
            self.protocol_section, fg_color="transparent"
        )

        # Scheme type selection (NVT, NPT, NPAT, NPgT)
        self.scheme_type_frame = ctk.CTkFrame(
            self.charmm_gui_scheme_frame, fg_color="transparent"
        )
    ...
    def _create_stage_widgets(self):
        ...
        self.sync_scroll_frame = ctk.CTkFrame(
            self.stages_config_frame, fg_color="transparent"
        )
        ...
        self.stages_container = ctk.CTkFrame(
            self.stages_config_frame, fg_color="transparent"
        )
                ...
                def create_sync_yview(canvas_index, orig_cmd):
                    def sync_yview(*args):
                        ...
                        for j, other_frame in enumerate(self.column_frames):
                            if (
                                hasattr(other_frame, "_parent_canvas")
                                and self.original_scroll_commands[j] is not None
                            ):
                                try:
                    ...
                    def on_mousewheel(event):
                        ...
                        for j, other_frame in enumerate(self.column_frames):
                            if hasattr(other_frame, "_parent_canvas"):
                                try:
                                    other_frame._parent_canvas.yview_scroll(
                                        delta, "units"
                                    )
                                except Exception:
                                    pass  # Silently handle errors
        ...
        if hasattr(self, "column_frames"):
            for i, column_frame in enumerate(self.column_frames):
                try:
                    ...
                    if (
                        hasattr(self, "original_scroll_commands")
                        and i < len(self.original_scroll_commands)
                        and hasattr(column_frame, "_parent_canvas")
                    ):
                        column_frame._parent_canvas.yview = (
                            self.original_scroll_commands[i]
                        )
    ...
    def _create_stage_content(self, parent_frame, stage_key, stage_data):
        ...
        time_text_label = ctk.CTkLabel(
            time_ns_frame, text="Time:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        time_unit_label = ctk.CTkLabel(
            time_ns_frame, text="ns", width=30, anchor="w", font=FONTS["body"]
        )
        ...
        steps_text_label = ctk.CTkLabel(
            steps_frame, text="Steps:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        steps_unit_label = ctk.CTkLabel(
            steps_frame, text="steps", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        if stage_key == "Equilibration 1":
            ...
            minimize_text_label = ctk.CTkLabel(
                minimize_frame,
                text="Minimize:",
                width=120,
                anchor="w",
            ...
            minimize_var = ctk.StringVar(
                value=str(stage_data.get("minimize_steps", 125000))
            )
            minimize_entry = ctk.CTkEntry(
                minimize_frame, textvariable=minimize_var, width=100
            )
            minimize_entry.pack(side="left", padx=5)

            minimize_info_label = ctk.CTkLabel(
                minimize_frame, text="steps", width=120, anchor="w", font=FONTS["small"]
            )
        ...
        temp_text_label = ctk.CTkLabel(
            temp_frame, text="Temperature:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        temp_unit_label = ctk.CTkLabel(
            temp_frame, text="K", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        pressure_text_label = ctk.CTkLabel(
            pressure_frame, text="Pressure:", width=120, anchor="w", font=FONTS["body"]
        )
        pressure_text_label.pack(side="left")
        pressure_var = ctk.StringVar(value=str(stage_data.get("pressure", 1.0)))
        pressure_entry = ctk.CTkEntry(
            pressure_frame, textvariable=pressure_var, width=100
        )
        pressure_entry.pack(side="left", padx=5)
        pressure_unit_label = ctk.CTkLabel(
            pressure_frame, text="bar", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        tension_text_label = ctk.CTkLabel(
            tension_frame,
        ...
        tension_unit_label = ctk.CTkLabel(
            tension_frame, text="dyn/cm", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        timestep_text_label = ctk.CTkLabel(
            timestep_frame, text="Timestep:", width=120, anchor="w", font=FONTS["body"]
        )
        timestep_text_label.pack(side="left")
        timestep_var = ctk.StringVar(value=str(stage_data["timestep"]))
        timestep_entry = ctk.CTkEntry(
            timestep_frame, textvariable=timestep_var, width=100
        )
        timestep_entry.pack(side="left", padx=5)
        timestep_unit_label = ctk.CTkLabel(
            timestep_frame, text="fs", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        dcd_freq_text_label = ctk.CTkLabel(
            dcd_freq_frame,
            text="DCD Frequency:",
            width=120,
            anchor="w",
        ...
        dcd_freq_entry = ctk.CTkEntry(
            dcd_freq_frame, textvariable=dcd_freq_var, width=100
        )
        dcd_freq_entry.pack(side="left", padx=5)
        dcd_freq_unit_label = ctk.CTkLabel(
            dcd_freq_frame, text="steps", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        margin_text_label = ctk.CTkLabel(
            margin_frame, text="Margin:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        margin_unit_label = ctk.CTkLabel(
            margin_frame, text="Å", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        gpu_text_label = ctk.CTkLabel(
            gpu_frame, text="Use GPU:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        cpu_text_label = ctk.CTkLabel(
            cpu_frame, text="CPU Cores:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        cpu_unit_label = ctk.CTkLabel(
            cpu_frame, text="cores", width=50, anchor="w", font=FONTS["body"]
        )
        ...
        gpu_id_text_label = ctk.CTkLabel(
            gpu_id_frame, text="GPU ID:", width=120, anchor="w", font=FONTS["body"]
        )
        ...
        gpu_id_hint_label = ctk.CTkLabel(
            gpu_id_frame, text="(device ID)", width=50, anchor="w", font=FONTS["body"]
        )
        gpu_id_hint_label.pack(side="left", padx=5)
        stage_widgets["gpu_id"] = gpu_id_var
        stage_widgets["gpu_id_entry"] = (
            gpu_id_entry  # Store widget reference for enable/disable
        )
        ...
        num_gpus_text_label = ctk.CTkLabel(
            num_gpus_frame,
            text="Number of GPUs:",
            width=120,
            anchor="w",
        ...
        num_gpus_entry = ctk.CTkEntry(
            num_gpus_frame, textvariable=num_gpus_var, width=100
        )
        num_gpus_entry.pack(side="left", padx=5)
        num_gpus_unit_label = ctk.CTkLabel(
            num_gpus_frame, text="GPUs", width=50, anchor="w", font=FONTS["body"]
        )
        num_gpus_unit_label.pack(side="left", padx=5)
        stage_widgets["num_gpus"] = num_gpus_var
        stage_widgets["num_gpus_entry"] = (
            num_gpus_entry  # Store widget reference for enable/disable
        )
    ...
    def _create_constraint_row(
        ...
        atom_count_label = ctk.CTkLabel(
            ...
            anchor="w",
            font=FONTS["small"],
            text_color=COLOR_SCHEME.get("inactive", "#888888"),
        )
        ...
        gear_btn = ctk.CTkButton(
            ...
            command=lambda sk=stage_key, cn=constraint_name: self._on_edit_selection(
                sk, cn
            ),
        )
    ...
    def _on_edit_selection(self, stage_key: str, constraint_name: str):
        ...
        current_sel = self.mda_selections.get(stage_key, {}).get(
            constraint_name,
            NAMDEquilibrationManager.DEFAULT_SELECTIONS.get(constraint_name, ""),
        )
        ...
        if dialog.result is not None:
            self.mda_selections.setdefault(stage_key, {})[
                constraint_name
            ] = dialog.result
    ...
    def _create_action_section(self):
        ...
        self.action_section = ctk.CTkFrame(
            self.main_scroll, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.action_buttons_frame = ctk.CTkFrame(
            self.action_section, fg_color="transparent"
        )
    ...
    def _create_progress_section(self):
        ...
        self.monitoring_controls = ctk.CTkFrame(
        ...
        self.interval_label = ctk.CTkLabel(
            self.monitoring_controls, text="Update every:", font=FONTS["body"]
        )

        self.interval_var = ctk.StringVar(value="5")
        self.interval_entry = ctk.CTkEntry(
            self.monitoring_controls, textvariable=self.interval_var, width=50
        )

        self.interval_unit_label = ctk.CTkLabel(
            self.monitoring_controls, text="seconds", font=FONTS["body"]
        )
        ...
        self.horizontal_progress_frame = ctk.CTkFrame(
            self.progress_section, fg_color="transparent"
        )
        ...
        for stage in self.stage_names:
            stage_frame = ctk.CTkFrame(
                self.horizontal_progress_frame,
                fg_color=COLOR_SCHEME["content_inside_bg"],
            )
            ...
            # Progress bar
            progress_bar = ctk.CTkProgressBar(stage_frame, width=150, height=15)
            progress_bar.set(0)
        ...
        self.summary_frame = ctk.CTkFrame(
            self.progress_section, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
    ...
    def _setup_layout(self):
        ...
        for i, stage in enumerate(self.stage_names):
            row = i // 3
            col = i % 3
            self.progress_widgets[stage]["frame"].grid(
    ...
    def _browse_inputfolder(self):
        ...
        directory = filedialog.askdirectory(
            title="Select Input Folder (Prepared System Files)", initialdir=initial_dir
        )
            ...
            if missing_files:
                ...
                if self.status_callback:
                    self.status_callback(
                        f"[OK] Input folder validated: {input_folder.name}"
                    )
    ...
    def _on_outputname_changed(self, event=None):
        ...
        if new_name and new_name != self.equilibration_output_name:
            ...
            if self.monitoring_active and hasattr(
                self, "_refresh_equilibration_progress"
            ):
                self._refresh_equilibration_progress()
    ...
    def _create_namd_settings(self):
        ...
        namd_path_frame = ctk.CTkFrame(
            self.engine_settings_frame, fg_color="transparent"
        )
        namd_path_frame.pack(fill="x", pady=2)

        ctk.CTkLabel(
            namd_path_frame,
            text="NAMD executable:",
            width=120,
            anchor="w",
            font=FONTS["body"],
        ).pack(side="left")
        self.namd_path_var = ctk.StringVar(value="namd3")
        namd_path_entry = ctk.CTkEntry(
            namd_path_frame, textvariable=self.namd_path_var, width=200
        )
    ...
    def _load_charmm_gui_protocol(self):
        ...
        if self.status_callback:
            self.status_callback(
                f"Loaded CHARMM-GUI {scheme_type} protocol with 13 equilibration stages"
            )
    ...
    def _refresh_equilibration_progress(self):
        ...
        try:
            ...
            equilibration_dir = (
                Path(self.working_directory) / self.equilibration_output_name
            )
    ...
    def _get_progress_stage_name(self, display_name: str) -> str:
        ...
        return display_to_internal.get(
    ...
    def _update_horizontal_progress(self, progress_dict):
        ...
        for stage_name in self.stage_names:
            ...
            if internal_name in progress_dict:
                ...
                if progress.timing:
                    ...
                    if progress.timing.simulated_time_ns > 0:
                        ...
                        if target_ns > 0:
                            ...
                            steps_text += (
                                f" ({progress.timing.simulated_time_ns:.3f} ns)"
                            )
                ...
                widgets["status_label"].configure(
                    text="No Data", text_color=COLOR_SCHEME["inactive"]
                )
                # Show target nanoseconds even for stages with no data
                target_ns = self._get_stage_target_ns(stage_name)
                if target_ns > 0:
                    widgets["steps_label"].configure(
                        text=f"0/0 steps (target: {target_ns:.3f} ns)"
                    )
                else:
    ...
    def _get_stage_target_ns(self, stage_name: str) -> float:
        ...
        try:
                    ...
                    if time_ns_str:
                        ...
                        if stage_name == "Equilibration 1":
                            ...
                            if "minimize_steps" in stage_widgets:
                                minimize_steps_str = (
                                    stage_widgets["minimize_steps"].get().strip()
                                )
            ...
            if stage_name in current_protocol:
                ...
                # For Equilibration 1, add minimize time to the target
                if stage_name == "Equilibration 1":
                    minimize_steps = current_protocol[stage_name].get(
    ...
    def _read_target_ns_from_config_file(self, stage_name: str) -> float:
        ...
        try:
            ...
            working_dir = (
                Path(self.working_directory) if self.working_directory else Path.cwd()
            )
            ...
            if time_match:
                ...
                r"run\s+\[expr\s+int\(\$time\s*\*\s*1e6\s*/\s*\$tstep\)\]", content
            ):
            ...
            self.logger.debug(
                f" Error reading target ns from config file for {stage_name}: {e}"
            )
            return 0.0
    ...
    def _update_summary_info(self, progress_dict):
        ...
        if timing_info:
            ...
            if timing_info.real_time_hours > 0:
                summary_lines.append(
                    f"Runtime: {timing_info.real_time_hours:.2f} hours"
                )
            summary_lines.append("")
        ...
        for stage_name in self.stage_names:
            ...
            if internal_name in progress_dict:
                ...
                summary_lines.append(
                    f"{status_icon} {stage_name}: {progress.status.replace('_', ' ').title()}"
                )

                if progress.timing and progress.timing.total_steps > 0:
                    percent = (
                        progress.timing.steps_completed / progress.timing.total_steps
                    ) * 100
                    summary_lines.append(
                        f"   Progress: {progress.timing.steps_completed}/{progress.timing.total_steps} steps ({percent:.1f}%)"
                    )
    ...
    def _format_equilibration_summary(self, progress_dict):
        ...
        completed_stages = sum(
            1 for p in progress_dict.values() if p.status == "completed"
        )
        running_stages = sum(1 for p in progress_dict.values() if p.status == "running")

        lines.append(
            f"📊 Overview: {completed_stages}/{total_stages} stages completed, {running_stages} running"
        )
        lines.append("")
        ...
        for stage_name, progress in progress_dict.items():
            ...
            if progress.timing:
                ...
                if timing.total_steps > 0:
                    ...
                    lines.append(
                        f"   Progress: {timing.steps_completed:,}/{timing.total_steps:,} steps ({progress_pct:.1f}%)"
                    )
                ...
                if timing.simulated_time_ns > 0:
                    lines.append(
                        f"   Simulated time: {timing.simulated_time_ns:.3f} ns"
                    )
            ...
            if progress.log_file:
                ...
                lines.append(
                    f"   📄 Log: {progress.log_file.name} (updated: {mod_time.strftime('%H:%M:%S')})"
                )
            else:
    ...
    def _on_generate_done(self, engine: str, error: Optional[Exception]):
        ...
        if error:
            ...
            messagebox.showerror(
                "Error", f"Failed to generate input files: {str(error)}"
            )
        else:
            if self._set_status_ready:
                self._set_status_ready(f"Generated {engine} input files")
            messagebox.showinfo(
                "Success", f"Input files generated successfully for {engine}!"
            )
    ...
    def _generate_namd_files(self, gui_snapshot: Optional[Dict[str, Any]] = None):
        ...
        output_dir = (
            Path(self.working_directory) / self.equilibration_output_name / "namd"
        )
        ...
        amber_files = list(source_dir.glob("**/*.top")) + list(
            source_dir.glob("**/*.inpcrd")
        )
        if amber_files:
            self.logger.info(
                "Found AMBER files from preparation, using AMBER force field"
            )
        else:
            self.logger.warning(
                "No AMBER files found. Please ensure AMBER topology and coordinate files are available."
            )
        ...
        prmtop_files = list(output_dir.glob("*.prmtop")) + list(
            output_dir.glob("*.top")
        )
        inpcrd_files = list(output_dir.glob("*.inpcrd")) + list(
            output_dir.glob("*.rst")
        )
        ...
        scheme_type = (
            gui_snapshot["scheme_type"] if gui_snapshot else self.scheme_type_var.get()
        )
        ...
        if not system_pdb.exists():
            ...
            self.logger.warning(
                f"system.pdb not found in output directory: {output_dir}"
            )
        ...
        if system_pdb and system_pdb.exists():
            ...
            try:
                ...
                self.logger.info(
                    f"Successfully created restraints directory: {restraints_dir}"
                )
            except OSError as e:
                self.logger.error(
                    f"Failed to create restraints directory {restraints_dir}: {e}"
                )
                # If restraints directory creation fails, continue without restraints
                self.logger.warning(
                    "Continuing without restraints due to directory creation failure"
                )
                restraints_dir = None
            ...
            if restraints_dir is not None:
                for i, (stage_key, stage_data) in enumerate(protocols.items()):
                    ...
                    has_restraints = any(
                        float(v) > 0 for v in stage_constraints.values()
                    )
                ...
                self.logger.info(
                    f"Generated general restraints file: {general_restraints}"
                )
            else:
                self.logger.warning(
                    "Skipping restraints file generation due to directory creation failure"
                )

        # Generate master run script
        run_script_content = namd_manager.generate_run_script(protocols, namd_exe)
    ...
    def _find_system_pdb(self, working_dir: Path) -> Optional[Path]:
        ...
        for prep_dir in sorted(
            prep_dirs, key=lambda x: x.stat().st_mtime, reverse=True
        ):
        ...
        if pdb_files:
            ...
            non_protein_files = [
                f for f in pdb_files if "protein" not in f.name.lower()
            ]
    ...
    def _find_original_bilayer_pdb(self, working_dir: Path) -> Optional[Path]:
        ...
        bilayer_patterns = ["bilayer_*.pdb", "*_bilayer.pdb"]
    ...
    def _copy_fallback_files(self, source_dir: Path, output_dir: Path):
        ...
        inpcrd_files = list(source_dir.glob("*.inpcrd")) + list(
            source_dir.glob("*.rst")
        )
        if inpcrd_files:
    ...
    def _copy_system_files_to_namd_dir(self, working_dir: Path, namd_dir: Path):
        ...
        if input_folder_str:
            ...
            self.logger.info(
                f"No input folder specified, searching in working directory: {source_dir}"
            )
        ...
        if input_folder_str:
            ...
            topology_files = list(source_dir.glob("*.prmtop")) + list(
                source_dir.glob("*.top")
            )
        else:
        ...
        if topology_files:
            ...
            self.logger.info(
                f"Copied topology file: {source_topology} -> {target_topology}"
            )
        else:
        ...
        if input_folder_str:
            # Look directly in the specified folder
            coordinate_files = list(source_dir.glob("*.inpcrd")) + list(
                source_dir.glob("*.rst")
            )
        else:
        ...
        if coordinate_files:
            ...
            self.logger.info(
                f"Copied coordinate file: {source_coords} -> {target_coords}"
            )
        else:
        ...
        if bilayer_pdb_files:
            ...
            self.logger.info(
                f"Copied bilayer PDB with CRYST1: {source_bilayer} -> {target_bilayer}"
            )
        else:
            self.logger.warning(
                f"No bilayer PDB with CRYST1 record (bilayer*_lipid.pdb) found in: {source_dir}"
            )
    ...
    def _run_equilibration(self):
        ...
        output_dir = (
            Path(self.working_directory) / self.equilibration_output_name / engine
        )

        if not output_dir.exists():
            messagebox.showerror(
                "Error", "Input files not found. Please generate input files first."
            )
            return
        ...
        try:
            ...
            if engine == "NAMD":
                ...
                self.after(
                    ...
                    lambda: messagebox.showinfo(
                        "Info", f"{engine_name} execution not yet implemented"
                    ),
                )
                return
            ...
            self.after(
                ...
                lambda: messagebox.showerror(
                    "Error", f"Failed to start equilibration: {error_msg}"
                ),
            )
    ...
    def _run_namd_equilibration(self):
        ...
        if not script_file.exists():
            raise FileNotFoundError(
                f"Run script not found: {script_file}. Please generate input files first."
            )
        ...
        try:
            ...
            self.logger.info(
                f"NAMD equilibration started as background process (PID: {process.pid})"
            )
    ...
    def _on_equilibration_started(self, process):
        ...
        if self.status_callback:
            self.status_callback(
                f"Equilibration running in background (PID: {process.pid})"
            )
    ...
    def _get_current_protocols(self) -> Dict[str, Any]:
        ...
        for stage_key, stage_widgets in self.stage_widgets.items():
            ...
            if stage_key == "Equilibration 1" and "minimize_steps" in stage_widgets:
                stage_data["minimize_steps"] = int(
                    stage_widgets["minimize_steps"].get()
                )
            ...
            if "surface_tension" in stage_widgets:
                stage_data["surface_tension"] = float(
                    stage_widgets["surface_tension"].get()
                )
            ...
            if "force_constant" in stage_widgets:
                stage_data["force_constant"] = float(
                    stage_widgets["force_constant"].get()
                )
    ...
    def _update_protocol_widgets(self, protocols: Dict[str, Any]):
        ...
        for stage_key, stage_data in protocols.items():
            if stage_key in self.stage_widgets:
                ...
                # Update minimize steps only for Equilibration 1
                if (
                    stage_key == "Equilibration 1"
                    and "minimize_steps" in stage_widgets
                    and "minimize_steps" in stage_data
                ):
                    stage_widgets["minimize_steps"].set(
                        str(stage_data["minimize_steps"])
                    )
                ...
                if (
                    "surface_tension" in stage_widgets
                    and "surface_tension" in stage_data
                ):
                    stage_widgets["surface_tension"].set(
                        str(stage_data["surface_tension"])
                    )
                ...
                # Update constraints
                for constraint_name, constraint_value in stage_data[
                    "constraints"
                ].items():
                    if constraint_name in stage_widgets["constraints"]:
                        stage_widgets["constraints"][constraint_name].set(
    ...
    def _check_amber_files(self, output_dir: Path):
        ...
        if missing_files:
            ...
            status_content += (
                "These files should be generated during the system preparation stage.\n"
            )
        else:
    ...
    def update_fonts(self, scaled_fonts):
        ...
        try:
            ...
            for attr in [
                "generate_files_btn",
                "run_equilibration_btn",
                "save_protocol_btn",
                "load_protocol_btn",
                "workdir_browse_btn",
                "inputfolder_browse_btn",
                ...
                if hasattr(self, attr):
                    safe_cfg(
                        getattr(self, attr),
                        scaled_fonts["body"],
                        width=WIDGET_SIZES.get("button_width", 120),
                        height=WIDGET_SIZES.get("button_height", 32),
                    )
            ...
            if hasattr(self, "summary_text"):
                safe_cfg(
                    self.summary_text, scaled_fonts.get("code", scaled_fonts["small"])
                )
