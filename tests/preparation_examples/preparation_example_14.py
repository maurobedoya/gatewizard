...
for i, (res_type, pka_vals, prot_states) in enumerate(
    zip(res_names, pka_values_list, protonation_list)
):
...
ax1.axhline(
    ...
    linewidth=2.5,
    alpha=0.8,
    label=f"pH {target_ph} (target)",
    zorder=2,
)

ax1.set_ylabel("pKa", fontsize=13, fontweight="bold")
ax1.set_title(
    f"pKa Distribution and Protonation States at pH {target_ph}",
    fontsize=15,
    fontweight="bold",
)
...
ax2.set_title(
    f"Protonation State Distribution at pH {target_ph}", fontsize=14, fontweight="bold"
)
...
for res_type, data in sorted(residue_data.items()):
    ...
    print(
        f"      Protonated: {n_prot}/{n_total} ({prot_frac*100:.0f}%)  "
