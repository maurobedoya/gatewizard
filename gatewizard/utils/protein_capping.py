...
class ProteinCapper:
    ...
    def remove_hydrogens_and_cap(
        ...
        if output_file is None:
            output_file = input_path.with_name(
                f"{input_path.stem}_capped{input_path.suffix}"
            )
        ...
        try:
            ...
            residue_mapping = self._add_capping_groups(
                temp_no_h_file, output_file, input_file
            )
        ...
        try:
            ...
            for i, seg in enumerate(u.segments):  # type: ignore[union-attr]
                if i < len(chain_list):
                    ...
                    logger.warning(
                        f"More segments than original chains, using segment {seg.segid} as chain ID"
                    )
    ...
    def _generate_gatewizard_mapping_file(
        self,
        ...
        try:
            ...
            for (original_resname, chain, original_resid), (
                final_resname,
                final_chain,
                final_resid,
            ) in residue_mapping.items():
            ...
            for chain in sorted(chains_data.keys()):
                chain_residues = sorted(
                    chains_data[chain], key=lambda x: x["original_resid"]
                )
                ...
                if chain_residues:
                    ...
                    last_final_resid = max(
                        r["final_resid"] for r in chain_residues if not r["is_cap"]
                    )
            ...
            with open(mapping_file, "w") as f:
                ...
                for chain in sorted(chains_data.keys()):
                    ...
                    chain_residues = sorted(
                        chains_data[chain], key=lambda x: x["final_resid"]
                    )
    ...
    def _get_ace_positions(self, n_term_residue) -> List[np.ndarray]:
        ...
        try:
            ...
            ch3_position = self._calc_coordinate(
                pos_ca, pos_n, c_position, 1.52, 120, 180
            )
    ...
    def _get_nme_positions(self, c_term_residue) -> List[np.ndarray]:
        ...
        try:
            ...
            bond_length = 1.45
            c_position = self._calc_coordinate(
                pos_o, pos_c, n_position, bond_length, 120, 0
            )
