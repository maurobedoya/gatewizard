...
class LeafletFrame(ctk.CTkFrame):
    ...
    def _setup_layout(self):
        ...
        self.add_button.pack(
            padx=LAYOUT["padding_medium"], pady=LAYOUT["padding_small"]
        )
    ...
    def _add_lipid_selection(self):
        ...
        ratio_label = ctk.CTkLabel(
            lipid_frame, text="Ratio:", font=FONTS["small"], width=40
        )
    ...
    def set_selected_lipids(
        self, lipids: List[str], ratios: Optional[List[float]] = None
    ):
        """
    ...
    def validate_selections(self) -> tuple[bool, str]:
        ...
        invalid_lipids = [
            lipid for lipid in selected if lipid not in self.available_lipids
        ]
        if invalid_lipids:
            return (
                False,
                f"Invalid lipids in {self.title.lower()}: {', '.join(invalid_lipids)}",
            )
        ...
        for i, ratio in enumerate(ratios):
            if ratio <= 0:
                return (
                    False,
                    f"Invalid ratio for {selected[i]} in {self.title.lower()}: must be positive",
                )
            ...
            if hasattr(self, "lipid_widgets"):
                for widget_dict in self.lipid_widgets:
                    if "remove_button" in widget_dict:
                        try:
                            widget_dict["remove_button"].configure(
                                font=scaled_fonts["small"]
                            )
                        except:
                            pass
