...
COLOR_SCHEMES_UI = ["Element (CPK)", "Chain", "Secondary Structure", "Residue Nature"]
SELECTION_CRITERIA = [
    "All",
    "Protein",
    "Backbone",
...
def _cs_key(label: str) -> str:
    m = {
        ...
        "Residue Nature": "residue_nature",
        "Uniform": "element",
        "element": "element",
        "chain": "chain",
        "ss": "ss",
        "uniform": "uniform",
        "residue_nature": "residue_nature",
    }
...
class VisualizeFrame(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self.ctrl = ctk.CTkScrollableFrame(
            self.main_frame, width=220, label_text="Controls"
        )
        ...
        ctk.CTkButton(
            sec_load.content, text="Open PDB File", command=self._browse_pdb, height=34
        ).pack(pady=4, padx=8, fill="x")
        self.pdb_id_entry = ctk.CTkEntry(
            sec_load.content, placeholder_text="PDB ID (e.g. 1CRN)"
        )
        self.pdb_id_entry.pack(pady=2, padx=8, fill="x")
        ctk.CTkButton(
            sec_load.content, text="Download PDB", command=self._download_pdb, height=34
        ).pack(pady=4, padx=8, fill="x")
        ...
        self.info_label = ctk.CTkLabel(
            self.ctrl, text="Load a structure to begin", wraplength=200
        )
        ...
        ctk.CTkButton(
            ...
            font=("", 11),
            fg_color="gray35",
            command=self._auto_detect_molecules,
        ...
        self._ref_lines_btn = ctk.CTkButton(
            btn_row2,
            text="Ref. Lines",
            width=80,
            height=26,
        ...
        ctk.CTkLabel(meas_row2, text="Size:", font=("", 10), anchor="w").pack(
            side="left", padx=(4, 0)
        )
        self._meas_size_var = ctk.StringVar(value="14")
        ctk.CTkOptionMenu(
            meas_row2,
            variable=self._meas_size_var,
        ...
        ctk.CTkLabel(meas_row3, text="Line:", font=("", 10), anchor="w").pack(
            side="left", padx=(4, 0)
        )
        self._meas_width_var = ctk.StringVar(value="2")
        ctk.CTkOptionMenu(
            meas_row3,
            variable=self._meas_width_var,
            values=["1", "2", "3", "4", "5", "6"],
            width=50,
            height=24,
            font=("", 10),
        ...
        ctk.CTkLabel(lbl_frame, text="Size:", font=("", 10), anchor="w").pack(
            side="left", padx=(4, 0)
        )
        size_spin = ctk.CTkOptionMenu(
            lbl_frame,
            variable=self._label_size_var,
            values=[str(s) for s in [8, 10, 12, 14, 16, 18, 20, 24, 28]],
            width=55,
            height=24,
            font=("", 10),
        ...
        ctk.CTkLabel(
            row_ao, text="Ambient Occlusion", font=("", 11), width=130, anchor="w"
        ).pack(side="left")
        self._ssao_var = ctk.StringVar(value="off")
        self._ssao_switch = ctk.CTkSwitch(
            row_ao,
            text="",
            variable=self._ssao_var,
            onvalue="on",
            offvalue="off",
            width=40,
        ...
        ctk.CTkLabel(row_sh, text="Shadows", font=("", 11), width=130, anchor="w").pack(
            side="left"
        )
        ...
        ctk.CTkLabel(
            row_dc, text="Depth Cueing", font=("", 11), width=130, anchor="w"
        ).pack(side="left")
        ...
        self._depth_cue_slider = ctk.CTkSlider(
            ...
            from_=0.0,
            to=1.0,
            variable=self._depth_cue_var,
            width=100,
            command=self._on_depth_cue_change,
        )
        ...
        ctk.CTkLabel(
            row_persp, text="Perspective", font=("", 11), width=130, anchor="w"
        ).pack(side="left")
        ...
        sec_sel = CollapsibleSection(
            right_frame, "Selections", expanded=True, fill_vertical=True
        )
        sec_sel.pack(fill="both", expand=True)
        self.sel_panel = ctk.CTkScrollableFrame(
            sec_sel.content, width=280, fg_color="transparent"
        )
            ...
            if rep == "vdw":
                ...
                sel.actors.extend(
                    make_cartoon_actor(
                        ...
                        uniform_color=uc,
                        quality=q,
                        helix_w=sel.helix_width,
                        sheet_w=sel.sheet_width,
                        coil_w=sel.coil_width,
                        ss_colors=sel.ss_colors,
                    )
                )
            elif rep == "tube_ss":
                sub = self._sub_struct(sel.atom_indices)
    ...
    def _update_axes_position(self):
        ...
        if vis_indices:
            coords = _np.array([self.structure.atoms[i].coord for i in vis_indices])
        else:
    ...
    def _on_right_click_atom(self, cx, cy, event):
        ...
        if atom is not None:
            ...
            label_menu.add_command(
                label="Custom...", command=lambda a=atom: self._custom_label_dialog(a)
            )
        ...
        if self._atom_labels:
            ...
            for idx, (_actor, _coord, text) in enumerate(self._atom_labels):
                rm_menu.add_command(
                    label=text, command=lambda i=idx: self._remove_label(i)
                )
            rm_menu.add_separator()
            rm_menu.add_command(
                label="Remove ALL labels", command=self._remove_all_labels
            )
        ...
        if self._measure_actors:
            ...
            rm_meas.add_command(
                label="Remove ALL measurements", command=self._remove_all_measurements
            )
    ...
    def _custom_label_dialog(self, atom):
        ...
        ctk.CTkButton(d, text="OK", height=28, command=_ok).pack(
            pady=(8, 4), padx=12, fill="x"
        )
    ...
    def _on_label_color_pick(self):
        ...
        result = colorchooser.askcolor(
            initialcolor=f"#{cur[0]:02x}{cur[1]:02x}{cur[2]:02x}", title="Label Color"
        )
        if result[0] is None:
            return
        r, g, b = result[0]
        ...
        hex_color = result[1]
        self._label_color_btn.configure(fg_color=hex_color, hover_color=hex_color)
        for actor, _coord, _text in self._atom_labels:
    ...
    def _on_meas_color_pick(self):
        ...
        hex_color = result[1]
        self._meas_color_btn.configure(fg_color=hex_color, hover_color=hex_color)
    ...
    def _on_meas_line_color_pick(self):
        ...
        hex_color = result[1]
        self._meas_line_color_btn.configure(fg_color=hex_color, hover_color=hex_color)
    ...
    def _start_measure(self, mode: str):
        ...
        self.info_label.configure(text=f"Measurement: click {need} atoms ({mode})")
    ...
    def _handle_measure_pick(self, atom):
        ...
        names = " -> ".join(
            f"{a.chain_id}:{a.res_name}{a.res_id}:{a.name}" for a in self._measure_picks
        )
    ...
    def _refresh_sel_ui(self):
        ...
        for idx, sel in enumerate(self.selections):
            ...
            name_lbl = ctk.CTkLabel(
                row1, text=sel.name, font=("", 13, "bold"), cursor="hand2"
            )
            name_lbl.pack(side="left", padx=4)
            name_lbl.bind("<ButtonPress-1>", lambda e, i=idx: self._drag_start(e, i))
            name_lbl.bind("<B1-Motion>", self._drag_motion)
            name_lbl.bind("<ButtonRelease-1>", self._drag_end)
            name_lbl.bind(
                "<Double-Button-1>", lambda e, s=sel: self._dbl_click_center(s)
            )
            if sel.name != "All":
                ctk.CTkButton(
                    row1,
            ...
            ctk.CTkLabel(
                row2,
                text=f"{len(sel.atom_indices)} atoms",
                font=("", 10),
                text_color="gray55",
    ...
    def _make_swatch(self, parent, sel):
        ...
        if cs == "uniform" and sel.uniform_color:
            ...
            colors = [sel.carbon_color, (255, 13, 13), (48, 80, 248), (255, 255, 255)]
        else:
            colors = self._SWATCH_STRIPES.get(cs, self._SWATCH_STRIPES["element"])
        size = 24
        sw = tk.Canvas(
            parent,
            width=size,
    ...
    def _drag_repack(self, highlight_idx):
        ...
        if highlight_idx < len(self._drag_data["frames"]):
            self._drag_data["frames"][highlight_idx].configure(border_color="#5599ff")
    ...
    def _edit_selection(self, sel):
        ...
        if sel.criteria:
            ...
            ctk.CTkLabel(
                d, text=f"Current: {info}", font=("", 11), text_color="gray60"
            ).pack(pady=(6, 0), padx=12, anchor="w")

        ctk.CTkLabel(d, text="Re-select atoms:", font=("", 12)).pack(
            pady=(6, 1), padx=12, anchor="w"
        )
        crit_menu = ctk.CTkOptionMenu(
            d, values=["(keep current)"] + SELECTION_CRITERIA, height=28
        )
        ...
        def _show_extra(val, prefill=""):
            ...
            if val == "Chain...":
                ...
                ctk.CTkLabel(extra_frame, text=f"Available: {chains}").pack(
                    anchor="w", pady=2
                )
                _extra[0] = ctk.CTkEntry(
                    extra_frame, placeholder_text="Chain IDs (e.g. A B)"
                )
                ...
                _extra[0] = ctk.CTkEntry(
                    extra_frame, placeholder_text="e.g. A:50 or A:10-50,B:1-20"
                )
                _extra[0].pack(fill="x", pady=2)
                if prefill:
                    _extra[0].insert(0, prefill)
                ...
                _extra[0] = ctk.CTkEntry(
                    extra_frame, placeholder_text="e.g. protein and name CA"
                )
                _extra[0].pack(fill="x", pady=2)
                if prefill:
                    _extra[0].insert(0, prefill)
        ...
        def _apply():
            ...
            if crit != "(keep current)":
                extra_text = (
                    _extra[0].get().strip()
                    if crit
                    in ("Chain...", "Residue range...", "MDAnalysis expression...")
                    else ""
                )
                ...
                if indices:
                    ...
                    messagebox.showwarning("Warning", "No atoms matched", parent=d)
                    return
            d.destroy()
            self._safe_refresh_rebuild()
        ...
        ctk.CTkButton(
            d, text="Cancel", height=28, fg_color="gray40", command=d.destroy
    ...
    def _resolve_sel_criteria(self, crit, extra=""):
        ...
        if crit == "Chain...":
            chains = {
                c.strip().upper() for c in extra.replace(",", " ").split() if c.strip()
            }
    ...
    def _chg_cs(self, sel):
        ...
        d.attributes("-topmost", True)
        d.after(100, d.grab_set)
        ctk.CTkLabel(d, text="Color Scheme", font=("", 13, "bold")).pack(
            pady=(8, 4), padx=12
        )
        cs = {
            "Element (CPK)": "element",
        ...
        def _pick_uniform(dd=d):
            dd.destroy()
            init = "#%02X%02X%02X" % sel.uniform_color if sel.uniform_color else None
            c = colorchooser.askcolor(initialcolor=init, title="Pick uniform color")
            if c[0]:
        ...
        ctk.CTkButton(
            d,
            text="Pick Uniform Color...",
            height=30,
            fg_color="#5a3a6e",
            hover_color="#7a4a9e",
            command=_pick_uniform,
        ).pack(pady=2, padx=12, fill="x")

        def _pick_carbon(dd=d):
            dd.destroy()
            init = "#%02X%02X%02X" % sel.carbon_color if sel.carbon_color else None
            c = colorchooser.askcolor(initialcolor=init, title="Pick carbon color")
            if c[0]:
        ...
        ctk.CTkButton(
            d,
            text="Pick Carbon Color...",
            height=30,
            fg_color="#3a5a6e",
            hover_color="#4a7a9e",
            command=_pick_carbon,
        ).pack(pady=2, padx=12, fill="x")
        if sel.carbon_color:
            ctk.CTkButton(
                d,
                text="Reset Carbon Color",
                height=28,
                fg_color="gray40",
                command=lambda dd=d: (
    ...
    def _sel_settings(self, sel):
        ...
        # Quality
        q_lbl = ctk.CTkLabel(
            d, text=f"Quality: {QUALITY_LABELS[sel.quality - 1]}", font=("", 11)
        )
        ...
        _sep(d)
        ctk.CTkLabel(d, text="Size Parameters", font=("", 12, "bold")).pack(
            pady=(2, 2), padx=12, anchor="w"
        )
        size_sliders: dict = {}
        if rep == "vdw":
            size_sliders["atom_scale"] = _mk(
                "Atom Scale", 0.1, 3.0, 29, sel.atom_scale
            )[0]
        elif rep == "ball_stick":
            size_sliders["ball_scale"] = _mk(
                "Ball Scale", 0.05, 1.5, 29, sel.ball_scale
            )[0]
            size_sliders["bond_radius"] = _mk(
                "Bond Radius", 0.02, 0.5, 24, sel.bond_radius
            )[0]
        elif rep == "sticks":
            size_sliders["stick_radius"] = _mk(
                "Stick Radius", 0.05, 0.6, 22, sel.stick_radius
            )[0]
        elif rep in ("cartoon", "tube_ss"):
            size_sliders["helix_width"] = _mk(
                "Helix Width", 0.5, 5.0, 18, sel.helix_width
            )[0]
            size_sliders["sheet_width"] = _mk(
                "Sheet Width", 0.5, 5.0, 18, sel.sheet_width
            )[0]
            size_sliders["coil_width"] = _mk(
                "Coil Width", 0.1, 3.0, 14, sel.coil_width
            )[0]
        elif rep == "backbone":
            size_sliders["backbone_radius"] = _mk(
                "Backbone Radius", 0.05, 1.5, 14, sel.backbone_radius
            )[0]
        elif rep == "surface":
            size_sliders["opacity"] = _mk("Opacity", 0.1, 1.0, 18, sel.opacity)[0]
            size_sliders["surface_resolution"] = _mk(
                "Resolution", 16, 128, 14, float(sel.surface_resolution), ".0f"
            )[0]
            size_sliders["surface_radius"] = _mk(
                "Surface Radius", 0.02, 0.40, 19, sel.surface_radius, ".3f"
            )[0]
        ...
        if rep in ("cartoon", "tube_ss", "backbone"):
            _sep(d)
            ctk.CTkLabel(
                d, text="Secondary Structure Colors", font=("", 12, "bold")
            ).pack(pady=(2, 2), padx=12, anchor="w")
            ...
            for ss_key, ss_label in SS_LABELS.items():
                ...
                ctk.CTkLabel(row, text=ss_label, font=("", 11), width=90).pack(
                    side="left"
                )
                c = sel.ss_colors.get(ss_key, SS_COLORS.get(ss_key, (200, 200, 200)))
                ...
                def _pick_ss(k=ss_key, btn=sw):
                    cur = sel.ss_colors.get(k, SS_COLORS.get(k, (200, 200, 200)))
                    cur_hex = "#%02X%02X%02X" % tuple(cur)
                    cc = colorchooser.askcolor(
                        initialcolor=cur_hex,
                        title=f"Pick {SS_LABELS[k]} color",
                        parent=d,
                    )
                    if cc[0]:
        ...
        # Material
        _sep(d)
        ctk.CTkLabel(d, text="Material", font=("", 12, "bold")).pack(
            pady=(2, 2), padx=12, anchor="w"
        )
        ...
        spp_sl, spp_lbl = _mk("Spec Power", 1, 100, 99, sel.specular_power, ".0f")
        ...
        ctk.CTkButton(
            d, text="Cancel", height=30, fg_color="gray40", command=_cancel
        ).pack(padx=12, fill="x")
        ...
        def _apply():
            ...
            if len(new_ch) != 1:
                messagebox.showwarning(
                    "Warning", "Chain ID must be 1 character", parent=d
                )
                return
            count = 0
            ...
            d.destroy()
            messagebox.showinfo(
                "Done", f"Renamed {count} atoms from chain {old_ch} to {new_ch}"
            )
            self._safe_refresh_rebuild()

        ctk.CTkButton(
            d, text="Apply", height=30, fg_color="#2a6e2a", command=_apply
        ).pack(pady=(10, 4), padx=12, fill="x")
    ...
    def _edit_residue_name(self):
        ...
        ctk.CTkLabel(d, text="New residue name:").pack(pady=(4, 1), padx=12, anchor="w")
    ...
    def _edit_residue_numbering(self):
        ...
        rng_entry.pack(padx=12, fill="x")
        ctk.CTkLabel(d, text="New start number:").pack(pady=(4, 1), padx=12, anchor="w")
        ...
        def _apply():
            ...
            d.destroy()
            messagebox.showinfo(
                "Done", f"Renumbered {count} atoms ({len(remap)} residues)"
            )
            self._safe_refresh_rebuild()
    ...
    def _transform_structure(self):
        ...
        # ---- Tabview -------------------------------------------------
        tabview = ctk.CTkTabview(d, height=380, segmented_button_fg_color="gray14")
        ...
        def _make_scope_selector(parent, label="Apply to"):
            ...
            ctk.CTkRadioButton(fr, text="All atoms", variable=mode, value="all").pack(
                anchor="w"
            )
            rb_sel = ctk.CTkRadioButton(
                fr, text="MDAnalysis selection", variable=mode, value="mda"
            )
            ...
            def _get():
                ...
                if not expr:
                    messagebox.showwarning(
                        "Warning", "Enter an MDAnalysis selection", parent=d
                    )
                    return None
        ...
        for ax in ("x", "y", "z"):
            ctk.CTkRadioButton(
                ax_rot_fr, text=ax.upper(), variable=axis_rot_var, value=ax, width=50
        ...
        ctk.CTkRadioButton(
            ctr_fr, text="Selection", variable=center_var, value="selection", width=90
        ).pack(side="left", padx=4)
        ctk.CTkRadioButton(
            ctr_fr, text="Origin", variable=center_var, value="origin", width=70
        ).pack(side="left", padx=4)
        ...
        def _apply_rot():
            ...
            try:
                ...
                messagebox.showwarning("Warning", "Enter a valid angle", parent=d)
                return
            atoms = self.structure.atoms
            coords = np.array([atoms[i].coord for i in idx])
            pivot = (
                coords.mean(axis=0) if center_var.get() == "selection" else np.zeros(3)
            )
            from gatewizard.core.viewer import _axis_rotation_matrix

            R = _axis_rotation_matrix(axis_rot_var.get(), np.radians(ang))
            for i in idx:
        ...
        def _preview_rot():
            ...
            R = _axis_rotation_matrix(axis_rot_var.get(), np.radians(ang))
            transformed = {i: R @ (atoms[i].coord - pivot) + pivot for i in idx}
            _clear_highlight()
            actors = self._make_highlight_actors(idx, coord_override=transformed)
            for a in actors:
        ...
        ctk.CTkButton(
            ...
            fg_color="#5a5a6e",
            hover_color="#7a7a9e",
            command=_preview_rot,
        ).pack(fill="x", pady=(0, 4))
        ctk.CTkButton(
            btn_rot, text="Apply", height=30, fg_color="#2a6e2a", command=_apply_rot
        ).pack(fill="x")
        ...
        def _apply_tra():
            ...
            try:
                ...
                messagebox.showwarning(
                    "Warning", "Enter valid displacement values", parent=d
                )
                return
            disp = np.array([dx, dy, dz])
            for i in idx:
                self.structure.atoms[i].coord = self.structure.atoms[i].coord + disp
            self.structure.build_bonds()
            self._reassign_ss_gui()
            _clear_highlight()
            self._safe_refresh_rebuild()
            messagebox.showinfo(
                "Done",
                f"Translated {len(idx)} atoms by " f"({dx:.2f}, {dy:.2f}, {dz:.2f}) Å",
                parent=d,
            )
        ...
        ctk.CTkButton(
            ...
            fg_color="#5a5a6e",
            hover_color="#7a7a9e",
            command=_preview_tra,
        ).pack(fill="x", pady=(0, 4))
        ctk.CTkButton(
            btn_tra,
            text="Center at Origin",
            height=28,
            fg_color="#5a5a6e",
            hover_color="#7a7a9e",
            command=_center_at_origin,
        ).pack(fill="x", pady=(0, 4))
        ctk.CTkButton(
            btn_tra, text="Apply", height=30, fg_color="#2a6e2a", command=_apply_tra
        ).pack(fill="x")

        # ==============================================================
        # ALIGN TAB
        # ==============================================================
        scope_ali_fr, scope_ali_get = _make_scope_selector(tab_ali, label="Transform")
        ...
        ctk.CTkLabel(pri_fr, text="Primary direction", font=("", 12, "bold")).pack(
            anchor="w"
        )
        ...
        pri_entry = ctk.CTkEntry(
            pri_fr, placeholder_text="e.g. name K or resid 60:80 and name CA"
        )
        ...
        for ax in ("x", "y", "z"):
            ctk.CTkRadioButton(
                pri_axis_fr, text=ax.upper(), variable=pri_axis_var, value=ax, width=50
            ).pack(side="left", padx=4)

        # --- secondary alignment (optional) ---------------------------
        sec_enabled = ctk.BooleanVar(value=False)
        sec_chk = ctk.CTkCheckBox(
            tab_ali,
            text="Secondary alignment",
        ...
        sec_entry = ctk.CTkEntry(sec_fr, placeholder_text="e.g. resid 45 and name CA")
        ...
        for ax in ("x", "y", "z"):
            ctk.CTkRadioButton(
                sec_axis_fr, text=ax.upper(), variable=sec_axis_var, value=ax, width=50
            ).pack(side="left", padx=4)
        ...
        def _apply_align():
            ...
            if not pri_expr:
                messagebox.showwarning(
                    "Warning", "Enter a primary direction selection", parent=d
                )
                return
            pri_idx = _resolve_mda_expression(self.structure, pri_expr, parent=d)
            if not pri_idx:
                return
            ...
            if sec_enabled.get():
                ...
                if not sec_expr:
                    messagebox.showwarning(
                        "Warning", "Enter a secondary direction selection", parent=d
                    )
                    return
                sec_idx = _resolve_mda_expression(self.structure, sec_expr, parent=d)
                if not sec_idx:
                    return
            ...
            pivot = np.array([atoms[i].coord for i in apply_idx]).mean(axis=0)
            ...
            self._safe_refresh_rebuild()
            msg = (
                f"Aligned {len(pri_idx)} atoms to "
                f"{target.upper()}-axis "
                f"({len(apply_idx)} atoms transformed)"
            )
            if sec_idx:
                msg += f"\nSecondary: {len(sec_idx)} atoms -> " f"{sec_ax.upper()}-axis"
            ...
            if sec_enabled.get():
                ...
                if sec_expr:
                    ...
                    if sec_idx and sec_ax != target:
                        sidx = axis_idx[sec_ax]
                        sec_coords = np.array(
                            [coords_copy.get(i, atoms[i].coord) for i in sec_idx]
                        )
                        ...
                        if norm > 1e-8:
                            ...
                            R2 = _axis_rotation_matrix(target, angle)
                            for i in apply_idx:
                                coords_copy[i] = R2 @ (coords_copy[i] - pivot) + pivot
            _clear_highlight()
            actors = self._make_highlight_actors(apply_idx, coord_override=coords_copy)
            for a in actors:
                self.vtk_frame.add_actor(a)
                _highlight_actors.append(a)
            self.vtk_frame.render()
        ...
        ctk.CTkButton(
            ...
            fg_color="#5a5a6e",
            hover_color="#7a7a9e",
            command=_preview_align,
        ).pack(fill="x", pady=(0, 4))
        ctk.CTkButton(
            btn_ali, text="Apply", height=30, fg_color="#2a6e2a", command=_apply_align
        ).pack(fill="x")
        ...
        ctk.CTkButton(
            d, text="Close", height=30, fg_color="gray40", command=_on_close
    ...
    def _delete_selection_atoms(self):
        ...
        ctk.CTkLabel(d, text="Delete Atoms", font=("", 14, "bold")).pack(
            pady=(10, 4), padx=12
        )
        ...
        crit_menu = ctk.CTkOptionMenu(panel_crit, values=SELECTION_CRITERIA, height=28)
        ...
        def _on_del_crit(val):
            ...
            if val == "Chain...":
                chains = ", ".join(sorted(self.structure.chains.keys()))
                ctk.CTkLabel(crit_extra_frame, text=f"Available: {chains}").pack(
                    anchor="w", pady=2
                )
                ...
                _crit_extra[0] = ctk.CTkEntry(
                    crit_extra_frame, placeholder_text="e.g. A:50 or A:10-50,B:1-20"
                )
                _crit_extra[0].pack(fill="x", pady=2)
                crit_extra_frame.pack(fill="x")
            elif val == "MDAnalysis expression...":
                ...
                hdr.pack(fill="x")
                ctk.CTkLabel(hdr, text="Expression:").pack(side="left", anchor="w")
                ctk.CTkButton(
                    hdr,
                    text="?",
                    width=28,
                ...
                _crit_extra[0] = ctk.CTkEntry(
                    crit_extra_frame, placeholder_text="e.g. resname LIG and name CA"
                )
                _crit_extra[0].pack(fill="x", pady=2)
                crit_extra_frame.pack(fill="x")
        ...
        names = (
            [s.name for s in self.selections[1:]]
        ...
        mda_entry = ctk.CTkEntry(
            panel_mda, placeholder_text="e.g. protein and not backbone"
        )
        mda_entry.pack(fill="x", pady=2)

        panels = {"criteria": panel_crit, "existing": panel_exist, "mda": panel_mda}
        ...
        def _gather_indices():
            ...
            if mode == "criteria":
                ...
                if crit == "MDAnalysis expression...":
                    ...
                    return _resolve_mda_expression(self.structure, expr, parent=d)
                extra = (
                    _crit_extra[0].get().strip()
                ...
                if chosen == "(no selections)":
                    return []
                sel = next((s for s in self.selections if s.name == chosen), None)
        ...
        def _do_delete():
            ...
            kept = []
            for sel in self.selections:
                new_indices = [idx_map[i] for i in sel.atom_indices if i in idx_map]
                if sel.name == "All":
        ...
        ctk.CTkButton(
            btn_frame, text="Cancel", height=30, fg_color="gray40", command=_on_cancel
        ).pack(fill="x")
    ...
    def _make_highlight_actors(self, indices, coord_override=None, offset=None):
    ...
    def _change_bg_color(self):
        ...
        ctk.CTkButton(d, text="Pick Color...", height=30, command=_pick).pack(
            pady=(12, 4), padx=12, fill="x"
        )
        ctk.CTkButton(
            d, text="Reset to Default", height=30, fg_color="gray40", command=_reset
        ...
        if t < 0.01:
            ...
            if not cam.GetParallelProjection():
                ...
                cam.SetParallelScale(
                    cam.GetDistance() * _m.tan(_m.radians(cam.GetViewAngle() / 2.0))
                )
            cam.SetParallelProjection(1)
        else:
    ...
    def _axes_dialog(self):
        ...
        mode_var = ctk.StringVar(value=self._axes_mode_current or "off")
        ...
        def _apply():
            ...
            if m == "center" and self.structure:
                ...
                if vis_indices:
                    ...
                    coords = _np.array([a.coord for a in self.structure.atoms])
                center = tuple(coords.mean(axis=0).tolist())
            self.vtk_frame.set_axes(mode=self._axes_mode_current, center=center)
            self.vtk_frame.render()
            c = "#4a7a4a" if self._axes_mode_current else "gray35"
            self._axes_btn.configure(fg_color=c)
            d.destroy()

        ctk.CTkButton(d, text="Apply", height=30, command=_apply).pack(
            pady=(10, 4), padx=12, fill="x"
        )
        ctk.CTkButton(
            d, text="Cancel", height=30, fg_color="gray40", command=d.destroy
        ).pack(pady=4, padx=12, fill="x")
    ...
    def _save_viewpoint(self):
        ...
        state = {
            "pdb_file": (
                os.path.abspath(self._pdb_filepath) if self._pdb_filepath else ""
            ),
        ...
        if path:
            try:
                ...
                messagebox.showinfo(
                    "Saved", f"Viewpoint saved to {os.path.basename(path)}"
                )
            except Exception as e:
    ...
    def _load_viewpoint(self):
        ...
        try:
            ...
            self.vtk_frame.render()
            messagebox.showinfo(
                "Loaded", f"Viewpoint loaded from {os.path.basename(path)}"
            )
        except Exception as e:
    ...
    def _save_image(self):
        ...
        scale_menu = ctk.CTkOptionMenu(d, values=["1x", "2x", "3x", "4x"], height=28)
        scale_menu.set("1x")
        ...
        bg_menu = ctk.CTkOptionMenu(
            d, values=["As displayed", "Transparent"], height=28
        )
        ...
        fmt_menu = ctk.CTkOptionMenu(
            d, values=["PNG", "JPEG", "TIFF", "BMP"], height=28
        )
        fmt_menu.set("PNG")
        ...
        def _do_save():
            ...
            fmt = fmt_menu.get()
            ext_map = {"PNG": ".png", "JPEG": ".jpg", "TIFF": ".tiff", "BMP": ".bmp"}
            ext = ext_map[fmt]
            d.destroy()
            ...
            if not save_path:
                return
            try:
                img = self.vtk_frame.render_to_image(
                ...
                img.save(save_path)
                messagebox.showinfo(
                    "Saved", f"Image saved to {os.path.basename(save_path)}"
                )
            except Exception as e:
                messagebox.showerror("Error", f"Save failed: {e}")
...
def _resolve_mda_expression(structure, expression, parent=None):
    ...
    try:
        ...
        messagebox.showerror(
            "Selection Error", f"Invalid MDAnalysis expression:\n{exc}", parent=parent
        )
        return []
    finally:
        try:
...
def _show_selection_help(parent):
    ...
    ctk.CTkLabel(
        hw, text="MDAnalysis Atom Selection Syntax", font=("", 15, "bold")
...
"""
    ...
    ctk.CTkButton(hw, text="Close", width=100, command=hw.destroy).pack(pady=(0, 10))
...
class SelectionDialog(ctk.CTkToplevel):
    def __init__(self, parent, structure, existing):
        ...
        self.name_entry = ctk.CTkEntry(
            self, placeholder_text="e.g. Active site", height=28
        )
        self.name_entry.pack(pady=(0, 4), padx=10, fill="x")

        ctk.CTkLabel(self, text="Select:").pack(pady=(4, 1), padx=10, anchor="w")
        self.crit_menu = ctk.CTkOptionMenu(
            self, values=SELECTION_CRITERIA, command=self._on_crit, height=28
        )
        ...
        self.rep_menu = ctk.CTkOptionMenu(self, values=REPRESENTATIONS_UI, height=28)
        self.rep_menu.pack(pady=(0, 2), padx=10, fill="x")
        self.rep_menu.set("Ball & Stick")

        ctk.CTkLabel(self, text="Color Scheme:").pack(pady=(4, 1), padx=10, anchor="w")
        self.cs_menu = ctk.CTkOptionMenu(self, values=COLOR_SCHEMES_UI, height=28)
        ...
        self._color_preview = ctk.CTkLabel(
            color_frame, text="  ", width=28, fg_color="gray40", corner_radius=4
        )
        ...
        ctk.CTkButton(
            bf, text="Cancel", command=self.destroy, height=30, fg_color="gray40"
        ).pack(side="left", expand=True, padx=4)

    def _pick_specific_color(self):
        c = colorchooser.askcolor(title="Pick color for this selection", parent=self)
        if c[0]:
    ...
    def _on_crit(self, val):
        ...
        if val == "Chain...":
            chains = ", ".join(sorted(self.structure.chains.keys()))
            ctk.CTkLabel(self.extra_frame, text=f"Available: {chains}").pack(
                anchor="w", pady=2
            )
            self.extra_entry = ctk.CTkEntry(
                self.extra_frame, placeholder_text="Chain IDs (e.g. A B or A,B)"
            )
            self.extra_entry.pack(fill="x", pady=2)
            self.extra_frame.pack(fill="x", padx=10, before=self.rep_label)
        elif val == "Residue range...":
            self.extra_entry = ctk.CTkEntry(
                self.extra_frame, placeholder_text="e.g. A:50 or A:10-50,B:1-20"
            )
            self.extra_entry.pack(fill="x", pady=2)
            self.extra_frame.pack(fill="x", padx=10, before=self.rep_label)
            ...
            if self.existing:
                ...
                ctk.CTkLabel(self.around_frame, text="Base selection:").pack(
                    anchor="w", pady=2
                )
                self.around_sel = ctk.CTkOptionMenu(self.around_frame, values=names)
                self.around_sel.pack(fill="x", pady=2)
                ctk.CTkLabel(self.around_frame, text="Distance (A):").pack(
                    anchor="w", pady=2
                )
                ...
                self.around_frame.pack(fill="x", padx=10, before=self.rep_label)
        elif val == "MDAnalysis expression...":
            ...
            self.mda_entry = ctk.CTkEntry(
                self._mda_frame, placeholder_text="e.g. protein and name CA"
            )
    ...
    def _resolve(self, crit):
        ...
        if crit == "MDAnalysis expression...":
            ...
            return _resolve_mda_expression(
                self.structure, expr.get().strip(), parent=self
            )
        return []
