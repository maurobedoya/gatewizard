...
class JobMonitor:
    ...
    def scan_for_jobs(self, force: bool = False):
        ...
        for job_dir in job_dirs:
            ...
            try:
                ...
                if file_size == 0:
                    logger.debug(
                        f"Skipping empty status file (being written): {status_file}"
                    )
                    continue
                ...
                if job_id in self.jobs:
                    ...
                    if self.jobs[job_id].progress != old_progress:
                        logger.debug(
                            f"Job {job_id}: progress {old_progress:.2f} -> {self.jobs[job_id].progress:.2f}"
                        )

                    # Check for process status if job claims to be running
                    if (
                        self.jobs[job_id].status == JobStatus.RUNNING
                        and old_status == JobStatus.RUNNING
                    ):
                ...
                logger.debug(
                    f"Temporary JSON parse error for {status_file} (file may be updating): {e}"
                )
                continue
        ...
        try:
            ...
            if pid_file.exists():
                ...
                try:
                    ...
                    logger.warning(
                        f"Job {job_info.job_id} status is 'running' but process {pid} not found"
                    )
    ...
    def _check_completion_by_output(self, job_dir: Path, job_info: JobInfo):
        ...
        if found_outputs:
            logger.info(
                f"Job {job_info.job_id} appears complete - found output files: "
            ...
            try:
                ...
                if status_file.exists():
                    ...
                    logger.info(
                        f"Automatically marked job {job_info.job_id} as completed"
                    )

            except Exception as e:
                logger.error(
                    f"Error updating completion status for {job_info.job_id}: {e}"
                )
    ...
    def refresh_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if (
                job.progress != old_progress
                or job.steps_completed != old_steps
                or job.current_step != old_current_step
                or job.status != old_status
            ):
                logger.info(
                    f"Job {job_id} updated: progress={job.progress:.2f}, "
            ...
            # This is expected when file is being written - use debug level
            logger.debug(
                f"Temporary JSON parse error for job {job_id} (file may be updating): {e}"
            )
            return False
        except IOError as e:
    ...
    def cleanup_stale_jobs(self, max_age_seconds: float = 3600):
        ...
        """
        stale_jobs = [
            job_id for job_id, job in self.jobs.items() if job.is_stale(max_age_seconds)
        ]
