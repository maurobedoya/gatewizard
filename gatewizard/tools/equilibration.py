...
# ---------------------------------------------------------------------------
# Module-level helper shared by NAMD / GROMACS COM colvars generation
# ---------------------------------------------------------------------------


def _build_com_colvars_namd_style(
    atom_numbers: str,
    x0: float,
    y0: float,
    z0: float,
    ...
    """
    header = (
        f"; GROMACS Colvars — COM restraint\n; Generated by GateWizard\n"
        if engine == "gromacs"
        else "# NAMD Colvars — COM restraint\n# Generated by GateWizard\n"
    )
    comment = ";" if engine == "gromacs" else "#"

    lines = [
        header,
        f"{comment} Translation restraint on the geometric centre of {len(ag.atoms)} atoms.",
        f"{comment} Force constant: {com_k} kcal/mol/Å² (same units as positional restraints).",
...
class NAMDEquilibrationManager:
    ...
    def setup_namd_equilibration(
        ...
        output_dir = namd_dir  # kept for backward compatibility
        ...
        if add_com_restraint:
            ...
            if pdb_src and Path(pdb_src).exists():
                ...
                com_colvars_path = self.generate_com_colvars_file(
                ...
                if com_colvars_path:
                    self.logger.info(
                        "  COM colvars file generated. Add to your NAMD config:\n"
                        "    colvars on\n"
    ...
    def generate_com_colvars_file(
        ...
        *add_rotation_restraint* is True.  The initial centroid is computed
        ...
        try:
            ...
            content = _build_com_colvars_namd_style(
...
class GROMACSEquilibrationManager:
    ...
    def generate_run_script(
        ...
        The script sources ``/usr/local/gromacs/bin/GMXRC`` and orchestrates
        ``gmx grompp`` + ``gmx mdrun`` for minimisation, all equilibration
        ...
        """
        ndx_flag = f"-n {ndx_name}" if ndx_name else ""
        lines = [
            "#!/bin/bash",
        ...
        lines += [
            ...
            "${GMX} mdrun -v -deffnm step0_minimization || { echo 'Minimisation failed'; exit 1; }",
            "",
        ...
        for i in range(1, n_stages + 1):
            ...
            lines += [
                ...
                f"${{GMX}} mdrun -v -deffnm {curr} || {{ echo 'Stage {i} failed'; exit 1; }}",
                "",
            ]
        lines += [
            "# --- Step 7: Production ---",
            ...
            "${GMX} mdrun -v -deffnm step7_production || { echo 'Production failed'; exit 1; }",
            "",
    ...
    def generate_com_colvars_file(
        ...
        The generated file uses ``distanceXY`` and ``distanceZ`` collective
        variables (translation) plus an ``orientation`` CV (rotation).
        The initial center and reference positions are computed from *pdb_path*.
    ...
    def setup_gromacs_equilibration(
        ...
        if add_com_restraint:
            ...
            if pdb_for_com and pdb_for_com.exists():
                ...
                if com_colvars_path:
                    self.logger.info(
                        "  COM colvars file generated. To activate, add to "
                        "the relevant MDP file(s):\n"
                        "    colvars-active         = yes\n"
