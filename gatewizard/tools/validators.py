...
class SystemValidator:
    ...
    def validate_lipid_ratios(
        self, ratios_string: str, upper_count: int, lower_count: int
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if lower_count > 0:
                if len(leaflet_ratios) < 2:
                    return (
                        False,
                        "Missing ratios for lower leaflet (use // to separate upper and lower ratios)",
                    )
    ...
    def _validate_ratio_numbers(
        self, ratios_str: str, expected_count: int, leaflet_name: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if any(r <= 0 for r in ratios):
                return (
                    False,
                    f"All ratios for {leaflet_name} leaflet must be positive numbers",
                )
            ...
            return (
                False,
                f"Invalid ratio format in {leaflet_name} leaflet: must be numbers separated by ':'",
            )

    def validate_force_fields(
        self, water_model: str, protein_ff: str, lipid_ff: str
    ) -> Tuple[bool, str, bool]:
        """
    ...
    def validate_salt_parameters(
        self, concentration: str, cation: str, anion: str
    ) -> Tuple[bool, str]:
        """
    ...
    def validate_system_inputs(
        ...
        **kwargs,
        ...
        valid, error = self.validate_lipid_ratios(
            lipid_ratios, upper_count, lower_count
        )
        if not valid:
            return False, error
        ...
        if water_model and protein_ff and lipid_ff:
            valid, message, is_warning = self.validate_force_fields(
                water_model, protein_ff, lipid_ff
            )
            if not valid:
        ...
        if kwargs.get("add_salt", False):
            ...
            valid, error = self.validate_salt_parameters(str(salt_conc), cation, anion)
            if not valid:
                return False, error
    ...
    def validate_propka_inputs(
        self, pdb_file: str, ph: str, version: str = "3"
    ) -> Tuple[bool, str]:
        """
    ...
    def validate_positive_number(
        self, value_str: str, name: str = "value", max_value: Optional[float] = None
    ) -> Tuple[bool, str]:
        """
    ...
    def validate_membrane_dimensions(
        self, x_str: str, y_str: str, z_str: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
