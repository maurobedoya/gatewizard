...
class PreparationFrame(ctk.CTkFrame):
    ...
    def __init__(
        ...
        self.analysis_file = (
            None  # Track which file was actually used for analysis (original or capped)
        )
        ...
        self.protein_capper = (
            ProteinCapper()
        )  # Initialize directly since MDAnalysis is required
        ...
        self.custom_state_combos = (
            {}
        )  # Dictionary to store ComboBox widgets by residue_id

        # Warning system for when options change after analysis
        self.analysis_run = False  # Track if analysis has been run
        self.options_changed_after_analysis = (
            False  # Track if options changed after analysis
        )
    ...
    def _create_widgets(self):
        ...
        self.left_panel = ctk.CTkFrame(
            self.main_container, fg_color=COLOR_SCHEME["content_inside_bg"], width=500
        )
        ...
        self.file_section = ctk.CTkFrame(
            self.left_scroll, fg_color=COLOR_SCHEME["canvas"]
        )
        ...
        self.working_file_frame = ctk.CTkFrame(
            self.file_section, fg_color="transparent"
        )

        self.working_file_label = ctk.CTkLabel(
            self.working_file_frame, text="Current Working File:", font=FONTS["body"]
        )
        ...
        self.export_file_label = ctk.CTkLabel(
            self.export_file_frame, text="Export Protonated File:", font=FONTS["body"]
        )
        ...
        self.right_panel = ctk.CTkFrame(
            self.main_container, fg_color=COLOR_SCHEME["content_inside_bg"]
        )
        ...
        self.analysis_results_container = ctk.CTkFrame(
            self.right_panel, fg_color=COLOR_SCHEME["viewer_bg"]
        )
        ...
        self.ph_label = ctk.CTkLabel(
            self.params_frame, text="Target pH:", font=FONTS["body"]
        )

        self.ph_entry = ctk.CTkEntry(
            self.params_frame, width=80, height=WIDGET_SIZES["entry_height"]
        )
        ...
        self.disulfide_section = ctk.CTkFrame(
            self.left_scroll, fg_color=COLOR_SCHEME["canvas"]
        )
        ...
        self.disulfide_controls_frame = ctk.CTkFrame(
            self.disulfide_section, fg_color="transparent"
        )
        ...
        self.distance_entry = ctk.CTkEntry(
            self.disulfide_controls_frame, width=80, height=WIDGET_SIZES["entry_height"]
        )
        ...
        self.disulfide_display_frame = ctk.CTkFrame(
            self.disulfide_section, fg_color="transparent"
        )

        self.disulfide_list_label = ctk.CTkLabel(
            self.disulfide_display_frame, text="Detected S-S Bonds:", font=FONTS["body"]
        )

        self.disulfide_text = ctk.CTkTextbox(
            self.disulfide_display_frame, height=80, font=FONTS["small"]
        )
        ...
        self.actions_section = ctk.CTkFrame(
            self.left_scroll, fg_color=COLOR_SCHEME["canvas"]
        )
    ...
    def _create_results_section(self):
        ...
        self.results_detailed_section = ctk.CTkFrame(
            self.analysis_results_container, fg_color=COLOR_SCHEME["canvas"]
        )
        ...
        self.results_frame = ctk.CTkFrame(
            self.results_detailed_section, fg_color="transparent"
        )

        # Column headers with sorting functionality
        self.headers_frame = ctk.CTkFrame(
            self.results_frame, fg_color=COLOR_SCHEME["canvas"]
        )
        ...
        self.header_id = ctk.CTkButton(
            ...
            font=FONTS["body"],
            width=(
                120 if self.cap_protein_var.get() else 60
            ),  # Wider when capping is enabled
            height=30,
        ...
        self.header_custom = ctk.CTkLabel(
            self.headers_frame, text="Custom State", font=FONTS["body"], width=120
        )

        # Scrollable results area
        self.results_scroll = ctk.CTkScrollableFrame(
            self.results_frame, height=400, fg_color=COLOR_SCHEME["background"]
        )
    ...
    def _setup_layout(self):
        ...
        self.analysis_results_container.pack(
        ...
        def _on_mousewheel(event):
            ...
            while current and current != self:
                if isinstance(current, ctk.CTkScrollableFrame):
                    ...
                    ) or current == getattr(self, "results_scroll", None):
    ...
    def _on_cap_toggle(self):
        ...
        if hasattr(self, "protonable_residues") and self.protonable_residues:
            current_ph = (
                float(self.ph_entry.get().strip())
                if self.ph_entry.get().strip()
                else 7.0
            )
    ...
    def _run_analysis_thread(self, ph_str: str, cap_protein: bool = False):
        ...
        def analysis_worker():
            try:
                    ...
                    try:
                        if self.status_callback:
                            self.status_callback(
                                "Removing hydrogens and adding caps..."
                            )
                        ...
                        target_dir = (
                            export_path.parent
                            if export_path
                            else Path(self.initial_directory)
                        )

                        capped_file = (
                            target_dir / f"{pdb_path.stem}_capped{pdb_path.suffix}"
                        )

                        analysis_file, self.residue_mapping = (
                            self.protein_capper.remove_hydrogens_and_cap(
                        ...
                        logger.debug(
                            f"Residue mapping: {len(self.residue_mapping)} entries"
                        )

                        # Inform user about mapping file creation
                        export_path = (
                            Path(self.export_file_entry.get().strip())
                            if self.export_file_entry.get().strip()
                            else None
                        )
                        target_dir = (
                            export_path.parent
                        ...
                        mapping_filename = (
                            f"{capped_file.stem}_gatewizard_residue_mapping.txt"
                        )
                        mapping_file = target_dir / mapping_filename
                        if self.status_callback and mapping_file.exists():
                            self.status_callback(
                                f"✅ GateWizard residue mapping file created: {mapping_file.name}"
                            )
                        ...
                        try:
                            if self.winfo_exists():
                                self.after(
                                    ...
                                    f"Protein capping failed: {str(e)}",
                                )
                        except:
                            pass
                        return
                        ...
                        logger.error(
                            f"Unexpected error during protein capping: {e}",
                            exc_info=True,
                        )
                        try:
                            if self.winfo_exists():
                ...
                target_dir = (
                    export_path.parent if export_path else Path(self.initial_directory)
                )
                ...
                if not analysis_file:
                    ...
                    return
                pka_file = self.preparation_manager.run_analysis(
                    analysis_file, output_dir=str(target_dir)
                )

                # Extract summary to target directory with custom filename
                summary_file = self.preparation_manager.extract_summary(
                    pka_file, output_dir=str(target_dir)
                )
                ...
                try:
                    if self.winfo_exists():
                        self.after(
                            0, self._show_analysis_error, f"Unexpected error: {str(e)}"
                        )
                except:
                    pass
    ...
    def _get_residue_id_display(self, residue: Dict[str, Any]) -> str:
        ...
        for (orig_resname, orig_chain, orig_resid), (
            new_resname,
            new_chain,
            new_resid,
        ) in self.residue_mapping.items():
            if (
                new_resid == current_resid
                and new_chain == chain
                and new_resname == resname
            ):
    ...
    def _update_results(self, residues: List[Dict[str, Any]], ph: float):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback(
                    f"Analysis complete: {len(residues)} protonable residues"
                )
    ...
    def _create_result_row(
        self, residue: Dict[str, Any], ph: float, row_index: int, parent_frame
    ):
        ...
        residue_label = ctk.CTkLabel(
            row_frame, text=residue["residue"], font=FONTS["body"], width=80
        )
        ...
        chain_label = ctk.CTkLabel(
            row_frame, text=chain_text, font=FONTS["body"], width=60
        )
        chain_label.pack(side="left", padx=2)

        # pKa value
        pka_label = ctk.CTkLabel(
            row_frame, text=f"{residue['pka']:.2f}", font=FONTS["body"], width=80
        )
        pka_label.pack(side="left", padx=2)

        # Default state at pH
        default_state = self.preparation_manager.get_default_protonation_state(
            residue, ph
        )
        state_label = ctk.CTkLabel(
            row_frame, text=default_state, font=FONTS["body"], width=100
        )
        state_label.pack(side="left", padx=2)

        # Custom state selection
        available_states = self.preparation_manager.get_available_states(
            residue["residue"]
        )
        ...
        if available_states:
            ...
            state_combo = ctk.CTkComboBox(
                ...
                width=120,
                height=25,
                command=lambda value, r=residue: self._on_custom_state_changed(
                    r, value
                ),
            )
    ...
    def _update_all_combo_colors(self, ph: float):
        ...
        for residue_id, combo in self.custom_state_combos.items():
            ...
            for residue in self.protonable_residues:
                ...
                if chain:
                    current_residue_id = (
                        f"{residue['residue']}{residue['res_id']}_{chain}"
                    )
                else:
    ...
    def _sort_results(self, column: str):
        ...
        try:
            if column == "residue":
                ...
                sorted_residues = sorted(
                    self.protonable_residues,
                    key=lambda x: self.preparation_manager.get_default_protonation_state(
                        x, ph
                    ),
                    reverse=not self.sort_ascending,
                )
    ...
    def _update_sort_indicators(self):
        ...
        for col, header in headers.items():
            if col == self.sort_column:
                ...
                header.configure(
                    text=base_texts[col], image=arrow_img, compound="right"
                )
    ...
    def _on_custom_state_changed(self, residue: Dict[str, Any], new_state: str):
        ...
        if chain:
            residue_id = (
                f"{residue['residue']}{residue['res_id']}_{chain}"  # ASP80_A format
            )
        else:
            residue_id = f"{residue['residue']}{residue['res_id']}"
    ...
    def _update_summary(self, residues: List[Dict[str, Any]], ph: float):
        ...
        try:
            ...
            state_counts = {}
            for residue in residues:
                state = self.preparation_manager.get_default_protonation_state(
                    residue, ph
                )
...
"""
    ...
    def _apply_protonation_states(self):
        ...
        if not input_file or not Path(input_file).exists():
            messagebox.showerror(
                "File Error", "Analysis file not found. Please run analysis again."
            )
            return
        ...
        if not output_file:
            messagebox.showwarning(
                "No Export File", "Please specify an export file path."
            )
            return
        ...
        try:
            ...
            if output_path.name.endswith("_prepared.pdb"):
                ...
                final_output_file = str(
                    output_path.with_name(f"{output_path.stem}_prepared.pdb")
                )
            ...
            if self.status_callback:
                self.status_callback(
                    "Running pdb4amber to prepare PDB for Amber compatibility..."
                )
            ...
            try:
                ...
                if pdb4amber_result["success"]:
                    ...
                    logger.info(
                        f"Applied {residue_changes} residue state changes ({record_changes} PDB records), {disulfide_changes} disulfide bonds, and pdb4amber processing"
                    )
                ...
                if self.status_callback:
                    self.status_callback(
                        f"Applied {residue_changes} residue changes ({record_changes} PDB records) and {disulfide_changes} disulfide bonds (pdb4amber failed)"
                    )
                ...
                logger.warning(
                    f"Applied {residue_changes} residue state changes ({record_changes} PDB records) and {disulfide_changes} disulfide bonds, but pdb4amber failed"
                )
    ...
    def _export_results(self):
        ...
        try:
            ...
            export_text += (
                f"  Protein Capping (ACE/NME): {'Yes' if self.capping_used else 'No'}\n"
            )

            if self.capping_used and self.residue_mapping:
                export_text += (
                    f"  Residues affected by capping: {len(self.residue_mapping)}\n"
                )
            ...
            export_text += (
                f"  Total Protonable Residues: {len(self.protonable_residues)}\n"
            )
            if self.custom_states:
    ...
    def _reset_custom_states(self):
        ...
        if result:
            try:
                ...
                messagebox.showerror(
                    "Error", f"Failed to reset custom states: {str(e)}"
                )
    ...
    def _show_analysis_error(self, error_message: str):
        ...
        messagebox.showerror(
            "Analysis Error", f"Propka analysis failed:\n\n{error_message}"
        )
    ...
    def _detect_disulfide_bonds(self):
        ...
        if not self.current_pdb_file:
            messagebox.showerror(
                "Error", "No PDB file loaded for disulfide bond detection"
            )
        ...
        try:
            ...
            self.detected_disulfide_bonds = (
                self.preparation_manager.detect_disulfide_bonds(
                    self.current_pdb_file, distance_threshold
                )
            )
            ...
            if self.detected_disulfide_bonds:
                ...
                if self.status_callback:
                    self.status_callback(
                        f"✅ Detected {len(self.detected_disulfide_bonds)} disulfide bonds"
                    )
            else:
