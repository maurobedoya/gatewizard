...
from gatewizard.utils.config import (
    load_config,
    save_config,
    get_config_value,
    set_config_value,
...
class ProteinViewerApp(ctk.CTk):
    ...
    def _setup_window(self):
        ...
        desired_width = get_config_value(
            "gui.window_width", WINDOW_CONFIG["default_width"]
        )
        desired_height = get_config_value(
            "gui.window_height", WINDOW_CONFIG["default_height"]
        )

        # Validate and correct problematic saved dimensions
        if desired_width > max_safe_width or desired_height > max_safe_height:
            logger.warning(
                f"Saved window size ({desired_width}x{desired_height}) too large for screen, correcting..."
            )
            ...
            try:
                ...
                save_config(config)
                logger.info(
                    f"Updated config with corrected window size: {desired_width}x{desired_height}"
                )
            except Exception as e:
        ...
        logger.info(
            f"Setting window size to {width}x{height} (screen: {screen_width}x{screen_height})"
        )
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.splitlines():
                    if " connected" not in line:
                        continue
                    match = _re.search(r"(\d+)x(\d+)\+(\d+)\+(\d+)", line)
                    if match:
        ...
        try:
            ...
            def _monitor_enum_cb(hMonitor, hdcMonitor, lprcMonitor, dwData):
                ...
                if user32.GetMonitorInfoW(hMonitor, ctypes.byref(info)):
                    rc = info.rcMonitor
                    geo = (rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top)
            ...
            user32.EnumDisplayMonitors(None, None, MONITORENUMPROC(_monitor_enum_cb), 0)
            if primary_geo:
                monitors.insert(0, primary_geo)
            else:
                monitors.sort(key=lambda g: (g[0], g[1]))
    ...
    def _position_on_screen(self):
        ...
        try:
            ...
            window_height = (
                getattr(self, "_initial_height", None) or self.winfo_height()
            )
            ...
            if get_config_value("gui.remember_window_position", True):
                ...
                if (
                    ...
                    if (
                        saved_x > -tolerance
                        and saved_x < screen_width - tolerance
                        and saved_y > -tolerance
                        and saved_y < screen_height - tolerance
                        and saved_width > 200
                        and saved_height > 200
                        and saved_width < screen_width + tolerance
                        and saved_height < screen_height + tolerance
                    ):
    ...
    def _validate_final_position(self):
        ...
        try:
            ...
            if (
                actual_x < -500
                or actual_x > screen_width + 500
                or actual_y < -500
                or actual_y > screen_height + 500
            ):
                logger.warning(
                    f"Window positioned incorrectly at {actual_x},{actual_y}, correcting..."
                )
                self._restore_safe_geometry()
                return
            ...
            margin = 80
            if (
                actual_width > screen_width - margin
                or actual_height > screen_height - margin
            ):
                logger.warning(
                    f"Window size too large ({actual_width}x{actual_height}) for screen ({screen_width}x{screen_height}), adjusting..."
                )
    ...
    def _create_widgets(self):
        ...
        self.title_frame = ctk.CTkFrame(
            self.main_container, fg_color=COLOR_SCHEME["canvas"], height=60
        )

        # Create title content frame for better layout
        self.title_content_frame = ctk.CTkFrame(
            self.title_frame, fg_color="transparent"
        )
        ...
        self.stage_tabs = StageTabsContainer(
            self.main_container, stage_changed_callback=self._on_stage_changed
        )

        # Create content frame
        self.content_frame = ctk.CTkFrame(
            self.main_container, fg_color=COLOR_SCHEME["content_bg"]
        )
    ...
    def _setup_bindings(self):
        ...
        self.bind(
            "<Control-exclam>", self._handle_stage_shortcut_1
        )  # Shift+1 on some layouts
        self.bind(
            "<Control-at>", self._handle_stage_shortcut_2
        )  # Shift+2 on some layouts
        self.bind(
            "<Control-numbersign>", self._handle_stage_shortcut_3
        )  # Shift+3 on some layouts
        self.bind(
            "<Control-dollar>", self._handle_stage_shortcut_4
        )  # Shift+4 on some layouts
        self.bind(
            "<Control-percent>", self._handle_stage_shortcut_5
        )  # Shift+5 on some layouts
        ...
        self.bind(
            "<Control-equal>", self._increase_font_size
        )  # For keyboards without separate +
        ...
        self.bind(
            "<Control-KP_Subtract>", self._decrease_font_size
        ...
        if hasattr(self, "current_stage") and hasattr(self, "stage_frames"):
            ...
            if current_frame and hasattr(current_frame, "_safe_status_callback"):
                try:
                    ...
                    if (
                        hasattr(current_frame, "status_text_label")
                        and current_frame.status_text_label
                    ):
    ...
    def _set_status_ready(self, message: str = "Ready"):
        ...
        self.status_label.configure(
            text=message, bg=COLOR_SCHEME["canvas"], fg=COLOR_SCHEME["text"]
        )
    ...
    def _handle_open_file_shortcut(self, event):
        ...
        if self.current_stage == "Visualize":
            ...
            self._update_status(
                f"Ctrl+O only works in Visualize tab (currently in {self.current_stage})"
            )
            return "break"
    ...
    def _debug_keypress(self, event):
        ...
        if (
            event.state & 0x4 or event.state & 0x8 or event.keysym.startswith("F")
        ):  # Control or Alt pressed or Function key
            key_info = {
                "keysym": event.keysym,
            ...
            if event.state & 0x4:  # Control pressed
                self._update_status(
                    f"Ctrl+{event.keysym} pressed (keycode: {event.keycode})"
                )
            elif event.state & 0x8:  # Alt pressed
                self._update_status(
                    f"Alt+{event.keysym} pressed (keycode: {event.keycode})"
                )
    ...
    def _create_shortcut_test_dialog(self):
        ...
        status_label = ctk.CTkLabel(
            test_window, textvariable=status_var, font=FONTS["small"]
        )
...
For more information, visit the documentation.
        ...
        help_label = ctk.CTkLabel(
            help_window, text=help_text, font=FONTS["body"], justify="left"
        )
    ...
    def _show_settings(self):
        ...
        title_label = ctk.CTkLabel(
            scrollable_frame, text="Settings", font=FONTS["heading"]
        )
        ...
        font_title = ctk.CTkLabel(
            font_frame, text="Display Settings", font=FONTS["heading"]
        )
        font_title.pack(pady=(10, 5))

        font_label = ctk.CTkLabel(font_frame, text="Font Size:", font=FONTS["body"])
        ...
        logging_title = ctk.CTkLabel(
            logging_frame, text="Logging Settings", font=FONTS["heading"]
        )
        ...
        info_label = ctk.CTkLabel(
            path_frame, text="📁 Auto", font=FONTS["small"], width=80
        )
        ...
        name_label = ctk.CTkLabel(name_frame, text="Log file name:", font=FONTS["body"])
        name_label.pack(anchor="w", pady=(5, 0))

        name_var = ctk.StringVar(value=config.logging.log_file_name)
        name_entry = ctk.CTkEntry(name_frame, textvariable=name_var, font=FONTS["body"])
        ...
        level_label = ctk.CTkLabel(level_frame, text="Log level:", font=FONTS["body"])
        ...
        size_label = ctk.CTkLabel(
            size_frame, text="Max file size (MB):", font=FONTS["body"]
        )
        ...
        size_entry = ctk.CTkEntry(
            size_frame, textvariable=size_var, font=FONTS["body"], width=80
        )
        ...
        backup_label = ctk.CTkLabel(
            backup_frame, text="Backup files count:", font=FONTS["body"]
        )
        ...
        backup_entry = ctk.CTkEntry(
            backup_frame, textvariable=backup_var, font=FONTS["body"], width=80
        )
        ...
        # Close button
        close_button = ctk.CTkButton(
            button_frame, text="Close", command=settings_window.destroy
        )
    ...
    def _update_fonts(self, scale_factor):
        ...
        try:
            ...
            self._update_widget_font(
                self.app_descriptor_label, scaled_fonts["subtitle"]
            )
    ...
    def _update_widget_font(self, widget, font):
        ...
        try:
            ...
            logger.debug(
                f"Could not update font for widget {type(widget).__name__}: {e}"
            )
    ...
    def _validate_window_geometry(self):
        ...
        try:
            ...
            if (
                ...
                or height < 100
                or x < -5000
                or x > screen_width + 5000  # Clearly off-screen
                or y < -5000
                or y > screen_height + 5000
            ):

                logger.warning(
                    f"Window geometry appears invalid (x={x}, y={y}, w={width}, h={height}), restoring safe position"
                )
                self._restore_safe_geometry()
    ...
    def _restore_safe_geometry(self):
        ...
        try:
            ...
            logger.info(
                f"Restored window to safe geometry: {safe_width}x{safe_height}+{safe_x}+{safe_y}"
            )
    ...
    def _save_window_position(self):
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"Skipping save of invalid window position: {x},{y} {width}x{height}"
                )
                self._position_save_timer = None
                return
            ...
            try:
                if newstate is None:
                    ...
                    if newstate == "zoomed":
                        ...
                        if (
                            current_width > screen_width - 50
                            or current_height > screen_height - 50
                        ):
                            logger.info(
                                f"Window too large for maximize ({current_width}x{current_height} on {screen_width}x{screen_height}), resizing first"
                            )
                        ...
                        if "+" in newGeometry or "-" in newGeometry:
                            ...
                            if len(parts) >= 3:
                                try:
                                    ...
                                    if (
                                        x < -10000
                                        or x > screen_width + 10000
                                        or y < -10000
                                        or y > screen_height + 10000
                                    ):
                                        logger.warning(
                                            f"Rejecting invalid geometry: {newGeometry}"
                                        )
                                        logger.warning(
                                            f"Coordinates ({x},{y}) are outside reasonable bounds"
                                        )
        ...
        def health_check():
            try:
                ...
                if state == "normal":
                    ...
                    if (
                        x < -width + 50
                        or x > screen_width - 50
                        or y < -height + 50
                        or y > screen_height - 50
                    ):
                        logger.warning(
                            f"Window appears off-screen ({x},{y}), correcting..."
                        )
                        self._restore_safe_geometry()
    ...
    def _check_initial_window_size(self):
        ...
        try:
            ...
            if needs_resize:
                logger.info(
                    f"Initial window size too large ({current_width}x{current_height}), adjusting to {new_width}x{new_height}"
                )
        ...
        try:
            ...
            if current_state == "zoomed":
                ...
                if (
                    ...
                    logger.info(
                        f"Resizing oversized window before maximize: {current_width}x{current_height} -> smaller"
                    )
                    safe_width = min(current_width, screen_width - 100)
                    safe_height = min(current_height, screen_height - 100)
                    self.geometry(f"{safe_width}x{safe_height}")
        ...
        try:
            ...
            try:
                ...
                if current_state in ("normal", None):
                    ...
                    logger.info(
                        f"Not saving window position - window state is {current_state}"
                    )
            except Exception as e:
            ...
            try:
                ...
                if hasattr(self, "tk") and self.tk:
                    ...
                    self.tk.call(
                        "after", "info"
                    )  # This returns list of pending after callbacks
            except Exception:
                pass
        ...
        try:
            ...
            if hasattr(widget, "cleanup_callbacks"):
                try:
                    ...
                    logger.debug(
                        f"Error calling cleanup_callbacks on {type(widget).__name__}: {e}"
                    )
                ...
                if hasattr(widget, attr_name):
                    ...
                    if callback_id:
                        try:
                            ...
                            logger.debug(
                                f"Cancelled callback {attr_name} on {type(widget).__name__}"
                            )
                        except Exception as e:
                            logger.debug(
                                f"Could not cancel {attr_name} on {type(widget).__name__}: {e}"
                            )
                    ...
                    if isinstance(callback_list, list):
                        for callback_id in callback_list[
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Could not cancel callback from {attr_name}: {e}"
                                )
