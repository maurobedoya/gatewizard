...
class ProgressTracker(ctk.CTkFrame):
    ...
    def _refresh_cycle(self):
        ...
        if self.job_monitor:
            ...
            if active_jobs:
                refresh_interval = (
                    1000  # 1 second for active jobs for better responsiveness
                )
            else:
    ...
    def refresh_jobs(self):
        ...
        if self.job_monitor:
            ...
            logger.info(
                f"Manual refresh completed. Found {len(active_jobs)} active jobs."
            )
    ...
    def _update_jobs_display(self):
        ...
        # Get current jobs
        all_jobs = {
            **self.job_monitor.get_active_jobs(),
            **self.job_monitor.get_completed_jobs(),
        }
    ...
    def _create_job_widget(self, job_id: str, job_info: JobInfo):
        ...
        job_label = ctk.CTkLabel(
            header_frame, text=job_name, font=FONTS["body"], anchor="w"
        )
        ...
        # Progress text
        progress_text = (
            f"{job_info.progress * 100:.0f}% - {self._get_current_step_text(job_info)}"
        )
    ...
    def _update_job_widget(self, job_id: str, job_info: JobInfo):
        ...
        widgets["status_label"].configure(
            text=job_info.status.value.upper(), text_color=status_color
        )

        # Update progress
        progress_value = job_info.progress
        widgets["progress_bar"].set(progress_value)
        widgets["progress_bar"].configure(progress_color=status_color)

        # Update progress text with more detailed information
        progress_text = (
            f"{progress_value * 100:.0f}% - {self._get_current_step_text(job_info)}"
        )
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f"Could not open job details dialog:\n{str(e)}"
                )
            except Exception:
    ...
    def _open_job_folder(self, job_dir: str):
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                if is_wsl:
                    win_path = (
                        subprocess.check_output(["wslpath", "-w", str(job_dir)])
                        .decode()
                        .strip()
                    )
...
class JobDetailDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        self.info_label = ctk.CTkLabel(
            self.info_frame, text="Job Information", font=FONTS["heading"]
        )

        # Details text area
        self.details_text = ctk.CTkTextbox(
            self.info_frame, height=200, font=FONTS["small"]
        )
        ...
        self.command_label = ctk.CTkLabel(
            self.command_frame, text="Command", font=FONTS["heading"]
        )

        self.command_text = ctk.CTkTextbox(
            self.command_frame, height=80, font=FONTS["code"]
        )
        ...
        self.logs_label = ctk.CTkLabel(
            self.logs_frame, text="Recent Log Output", font=FONTS["heading"]
        )
        ...
        self.open_folder_button = ctk.CTkButton(
            self.buttons_frame, text="Open Folder", command=self._open_folder
        )

        self.copy_command_button = ctk.CTkButton(
            self.buttons_frame, text="Copy Command", command=self._copy_command
        )
...
"""
    ...
    def _copy_command(self):
        ...
        try:
            ...
            self.after(
                1000, lambda: self.copy_command_button.configure(text=original_text)
            )
...
class SimpleProgressBar(ctk.CTkFrame):
    ...
    def __init__(self, parent, width: int = 200, height: int = 20):
        ...
        """
        super().__init__(
            parent, fg_color=COLOR_SCHEME["inactive"], width=width, height=height
        )
        ...
        self.progress_fill = ctk.CTkFrame(
            self, fg_color=COLOR_SCHEME["highlight"], width=0, height=height - 4
        )
        self.progress_fill.place(x=2, y=2)

        # Progress text
        self.progress_text = ctk.CTkLabel(
            self, text="0%", font=FONTS["small"], text_color="white"
        )
...
class JobSummaryWidget(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        self.title_label = ctk.CTkLabel(self, text="Job Summary", font=FONTS["heading"])

        # Statistics frame
        self.stats_frame = ctk.CTkFrame(self, fg_color="transparent")

        # Running jobs
        self.running_frame = ctk.CTkFrame(
            self.stats_frame, fg_color=COLOR_SCHEME["highlight"]
        )
        self.running_count = ctk.CTkLabel(
            self.running_frame, text="0", font=FONTS["title"], text_color="white"
        )
        self.running_label = ctk.CTkLabel(
            self.running_frame, text="Running", font=FONTS["small"], text_color="white"
        )

        # Completed jobs
        self.completed_frame = ctk.CTkFrame(
            self.stats_frame, fg_color=COLOR_SCHEME["active"]
        )
        self.completed_count = ctk.CTkLabel(
            self.completed_frame, text="0", font=FONTS["title"], text_color="white"
        )
        ...
        self.failed_count = ctk.CTkLabel(
            self.failed_frame, text="0", font=FONTS["title"], text_color="white"
        )
        self.failed_label = ctk.CTkLabel(
            self.failed_frame, text="Failed", font=FONTS["small"], text_color="white"
        )
