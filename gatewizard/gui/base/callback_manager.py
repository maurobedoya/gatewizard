...
class CallbackManager:
    ...
    def schedule_callback(
        self, delay: int, callback: Callable, *args: Any
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if not hasattr(self, "after"):
                raise AttributeError(
                    "CallbackManager must be used with a Tkinter widget class"
                )
    ...
    def cleanup_callbacks(self) -> None:
        ...
        logger.debug(
            f"Cleaned up {len(self._callback_ids)} callbacks for {type(self).__name__}"
        )
