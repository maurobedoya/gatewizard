...
from gatewizard.gui.constants import COLOR_SCHEME, WIDGET_SIZES, LAYOUT
...
class AnalysisFrame(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.discovered_log_files = (
            {}
        )  # Store discovered log files with basename as key
        self.output_directory = tk.StringVar(
            value=os.getcwd()
        )  # Auto-assign to current directory
        ...
        self.reference_frame = tk.StringVar(
            value="0"
        )  # Changed to StringVar to avoid IntVar empty string errors
        self.align_rmsd = tk.BooleanVar(
            value=True
        )  # Align structures before RMSD calculation
        ...
        self.namd_log_columns = tk.StringVar(
            value=""
        )  # Selected columns for NAMD log analysis
        self.namd_available_columns = []  # Store available NAMD columns
        self.column_checkboxes = (
            {}
        )  # Dictionary to store checkbox variables for multi-column selection

        # Plot parameters
        self.plot_enabled = tk.BooleanVar(value=True)
        self.plot_title = tk.StringVar(value="RMSD Analysis")
        self.plot_xlabel = tk.StringVar(
            value="Time (ns)"
        )  # Updated to match default time unit
        self.plot_ylabel = tk.StringVar(
            value="RMSD (Å)"
        )  # Updated to match default distance unit
        self.plot_color = tk.StringVar(value="blue")
        self.plot_background_color = tk.StringVar(
            value="#2b2b2b"
        )  # Dark gray background
        self.plot_line_color = tk.StringVar(
            value="blue"
        )  # Line color (separate from overall color scheme)
        ...
        self.plot_units = tk.StringVar(
            value="Å"
        )  # Distance unit selection (Y-axis) - Default to Angstroms
        self.plot_time_units = tk.StringVar(
            value="ns"
        )  # Time unit selection (X-axis) - Default to nanoseconds
        self.plot_energy_units = tk.StringVar(
            value="kcal/mol"
        )  # Energy unit selection (Y-axis for energetic analysis)

        # RMSF-specific plot options
        self.rmsf_xaxis_type = tk.StringVar(
            value="residue_number"
        )  # Options: "residue_number", "residue_type_number", "atom_index"
        self.rmsf_show_residue_labels = tk.BooleanVar(
            value=True
        )  # Show residue labels on X-axis
        self.rmsf_residue_name_format = tk.StringVar(
            value="single"
        )  # Options: "single" (1-letter), "triple" (3-letter)
        self.rmsf_label_frequency = tk.StringVar(
            value="auto"
        )  # Options: "all", "auto", "every_2", "every_5", "every_10", "every_20"

        # Figure background color (the frame around the plot)
        self.plot_figure_bg_color = tk.StringVar(
            value="#212121"
        )  # Dark background for figure frame

        # Text and axes color control
        self.plot_text_color = tk.StringVar(
            value="Auto"
        )  # Options: "Auto" (based on luminance), preset colors, "Custom RGB"
        ...
        self.plot_yaxis_units = tk.StringVar(
            value="kcal/mol"
        )  # General Y-axis units (auto-updates based on data type)
        ...
        self.unit_systems = {
            "energy": {
                ...
                "default": "kcal/mol",
                "conversions": {"kcal/mol": 1.0, "kJ/mol": 4.184},
            },
            "temperature": {
                "units": ["K", "°C", "°F"],
                "default": "K",
        ...
        if MDANALYSIS_AVAILABLE:
            ...
            self.status_callback(
                "Analysis frame initialized - MDAnalysis not available"
            )
    ...
    def _setup_parameter_tracking(self):
        ...
        self.atom_selection.trace_add(
            "write", lambda *args: self._on_parameter_change()
        )
        self.custom_atom_selection.trace_add(
            "write", lambda *args: self._on_parameter_change()
        )

        # Track reference frame changes with validation
        self.reference_frame.trace_add(
            "write", lambda *args: self._on_reference_frame_change()
        )

        # Track distance analysis selections
        self.distance_selection1.trace_add(
            "write", lambda *args: self._on_parameter_change()
        )
        self.distance_selection2.trace_add(
            "write", lambda *args: self._on_parameter_change()
        )
        ...
        self.output_directory.trace_add(
            "write", lambda *args: self._on_parameter_change()
        )
    ...
    def create_analysis_subtabs(self):
        ...
        self.tab_content_frame.grid_rowconfigure(
            0, weight=2
        )  # File section gets more space
    ...
    def switch_analysis_tab(self, tab_name):
        ...
        if current_tab == "Structural":
            ...
            self.structural_state["plot_background_color"] = (
                self.plot_background_color.get()
            )
            ...
            if hasattr(self, "rmsd_data") and self.rmsd_data is not None:
                self.structural_state["rmsd_data"] = (
                    self.rmsd_data.copy()
                    if hasattr(self.rmsd_data, "copy")
                    else self.rmsd_data
                )
                if hasattr(self, "time_data"):
                    self.structural_state["time_data"] = (
                        self.time_data.copy()
                        if hasattr(self.time_data, "copy")
                        else self.time_data
                    )
            ...
            self.energetic_state["plot_background_color"] = (
                self.plot_background_color.get()
            )
            ...
            if hasattr(self, "namd_available_columns"):
                self.energetic_state["available_columns"] = (
                    self.namd_available_columns.copy()
                )
        ...
        if tab_name == "Structural":
            ...
            self.plot_background_color.set(
                self.structural_state["plot_background_color"]
            )
            ...
            if "available_columns" in self.energetic_state:
                self.namd_available_columns = self.energetic_state[
                    "available_columns"
                ].copy()
    ...
    def _restore_plot_for_tab(self, tab_name):
        ...
        if tab_name == "Structural":
            ...
            if (
                hasattr(self, "rmsd_data")
                and self.rmsd_data is not None
                and hasattr(self, "time_data")
            ):
    ...
    def create_structural_analysis_content(self):
        ...
        self.create_analysis_section_in_panel(
            current_row,
            analysis_types=["-", "RMSD", "RMSF", "Distances", "Radius of Gyration"],
        )
        current_row += 1
    ...
    def create_energetic_analysis_content(self):
        ...
        # Analysis settings section
        self.create_analysis_section_in_panel(
            current_row, analysis_types=["-", "NAMD Log Analysis"]
        )
        current_row += 1
    ...
    def create_file_section_in_panel(self, row, file_type="dcd"):
        ...
        self.file_title_label = ctk.CTkLabel(
            file_frame, text="Input Files", font=ctk.CTkFont(size=16, weight="bold")
        )
        self.file_title_label.grid(
        ...
        if file_type == "dcd":
            ...
            topology_entry = ctk.CTkEntry(
                file_frame, textvariable=self.topology_file, width=200
            )
        ...
        label_text = (
            "Trajectories (with simulation time):"
            if file_type == "dcd"
            else "Log Files (with simulation time):"
        )
        ...
        self.file_list_frame.grid(
            row=traj_row + 1, column=0, columnspan=3, pady=5, padx=10, sticky="ew"
        )
        self.file_list_frame.grid_columnconfigure(
            0, weight=1
        )  # Filename column expands
        self.file_list_frame.grid_columnconfigure(
            1, weight=0
        ...
        traj_buttons_frame.grid(
            row=traj_row + 2, column=0, columnspan=3, pady=5, padx=10, sticky="e"
        )
        ...
        output_entry = ctk.CTkEntry(
            file_frame, textvariable=self.output_directory, width=200
        )
    ...
    def create_analysis_section_in_panel(self, row, analysis_types=None):
        ...
        self.analysis_type_label = ctk.CTkLabel(
            self.settings_frame, text="Analysis Type:"
        )
        ...
        self.dynamic_options_frame = ctk.CTkFrame(
    ...
    def create_plot_section_in_panel(self, row):
        ...
        self.plot_title_label = ctk.CTkLabel(
            plot_frame, text="Plot Settings", font=ctk.CTkFont(size=16, weight="bold")
        )
        self.plot_title_label.grid(
            row=0, column=0, columnspan=2, pady=(10, 5), sticky="w", padx=10
        )
        ...
        plot_title_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_title, width=200
        )
        ...
        self.custom_line_color_entry = ctk.CTkEntry(
            line_color_frame, width=80, placeholder_text="#RRGGBB"
        )
        self.custom_line_color_entry.grid(row=0, column=1, padx=0)
        self.custom_line_color_entry.grid_remove()  # Hidden initially
        self.custom_line_color_entry.bind(
            "<KeyRelease>", self._safe_update_plot_wrapper
        )
        ...
        self.custom_bg_color_entry = ctk.CTkEntry(
            bg_color_frame, width=80, placeholder_text="#RRGGBB"
        )
        ...
        if current_tab == "Structural":
            ...
            self.plot_time_units_combo.grid(
                row=9, column=1, pady=5, padx=(5, 10), sticky="w"
            )
            ...
            self.rmsf_xaxis_label.grid(
                row=10, column=0, pady=5, padx=(10, 5), sticky="w"
            )
            ...
            self.rmsf_xaxis_combo.grid(
                row=10, column=1, pady=5, padx=(5, 10), sticky="w"
            )

            # Residue name format (1-letter vs 3-letter code)
            self.rmsf_format_label = ctk.CTkLabel(plot_frame, text="Residue Format:")
            self.rmsf_format_label.grid(
                row=11, column=0, pady=5, padx=(10, 5), sticky="w"
            )
            ...
            self.rmsf_format_combo.grid(
                row=11, column=1, pady=5, padx=(5, 10), sticky="w"
            )

            # Label frequency control
            self.rmsf_frequency_label = ctk.CTkLabel(
                plot_frame, text="Label Frequency:"
            )
            self.rmsf_frequency_label.grid(
                row=12, column=0, pady=5, padx=(10, 5), sticky="w"
            )
            ...
            self.rmsf_frequency_combo.grid(
                row=12, column=1, pady=5, padx=(5, 10), sticky="w"
            )
            ...
            self.rmsf_labels_checkbox.grid(
                row=13, column=0, columnspan=2, pady=5, padx=10, sticky="w"
            )
            ...
            plot_time_units_combo.grid(
                row=8, column=1, pady=5, padx=(5, 10), sticky="w"
            )
            ...
            self.plot_yaxis_units_combo.grid(
                row=9, column=1, pady=5, padx=(5, 10), sticky="w"
            )
            next_row = 10
        ...
        plot_xlabel_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlabel, width=200
        )
        plot_xlabel_entry.grid(
            row=next_row, column=1, pady=5, padx=(5, 10), sticky="ew"
        )
        ...
        plot_ylabel_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylabel, width=200
        )
        plot_ylabel_entry.grid(
            row=next_row + 1, column=1, pady=5, padx=(5, 10), sticky="ew"
        )
        ...
        plot_xlim_min_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlim_min, width=100
        )
        plot_xlim_min_entry.grid(
            row=next_row + 2, column=1, pady=5, padx=(5, 10), sticky="w"
        )
        ...
        plot_xlim_max_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlim_max, width=100
        )
        plot_xlim_max_entry.grid(
            row=next_row + 3, column=1, pady=5, padx=(5, 10), sticky="w"
        )
        ...
        plot_ylim_min_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylim_min, width=100
        )
        plot_ylim_min_entry.grid(
            row=next_row + 4, column=1, pady=5, padx=(5, 10), sticky="w"
        )
        ...
        plot_ylim_max_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylim_max, width=100
        )
        plot_ylim_max_entry.grid(
            row=next_row + 5, column=1, pady=(5, 10), padx=(5, 10), sticky="w"
        )
        ...
        reset_button.grid(
            row=next_row + 6, column=0, columnspan=2, pady=(10, 5), padx=10
        )
    ...
    def create_results_section_in_panel(self, row):
        ...
        self.results_info = ctk.CTkTextbox(
            results_frame, height=100, width=300, wrap="word"
        )
        self.results_info.grid(
            row=1, column=0, columnspan=2, pady=5, padx=10, sticky="ew"
        )
    ...
    def create_control_section_in_panel(self, row):
        ...
        if current_tab == "Structural":
            ...
            self.control_title_label.grid(
                row=0, column=0, pady=(10, 5), sticky="w", padx=10
            )
        ...
        try:
            ...
            if current_tab == "Structural":
                ...
                if hasattr(self, "analysis_results") and self.analysis_results:
                    if (
                        "rmsf" in self.analysis_results
                        and "residue_info" in self.analysis_results
                    ):
                        # We have RMSF data
                        try:
                            self._plot_rmsf_data(
                                self.analysis_results["rmsf"],
                ...
                if (
                    ...
                    and "timestep" in self.analysis_results
                ):
                    try:
                        self._plot_namd_data()
                    except Exception as e:
    ...
    def _safe_update_plot_wrapper(self, event=None):
        ...
        try:
            if (
                hasattr(self, "_plot_update_in_progress")
                and self._plot_update_in_progress
            ):
    ...
    def _on_distance_unit_change(self, value):
        ...
        if "nm" in current_ylabel.lower():
            new_ylabel = (
                current_ylabel.replace("nm", value)
                .replace("Nm", value)
                .replace("NM", value)
            )
        elif "å" in current_ylabel.lower() or "angstrom" in current_ylabel.lower():
            new_ylabel = (
                current_ylabel.replace("Å", value)
                .replace("å", value)
                .replace("angstrom", value)
                .replace("Angstrom", value)
            )
        else:
    ...
    def _on_time_unit_change(self, value):
        ...
        if "ps" in current_xlabel.lower():
            new_xlabel = (
                current_xlabel.replace("ps", value)
                .replace("Ps", value)
                .replace("PS", value)
            )
        elif "ns" in current_xlabel.lower():
            new_xlabel = (
                current_xlabel.replace("ns", value)
                .replace("Ns", value)
                .replace("NS", value)
            )
        elif "µs" in current_xlabel.lower() or "us" in current_xlabel.lower():
            new_xlabel = (
                current_xlabel.replace("µs", value)
                .replace("us", value)
                .replace("Us", value)
                .replace("US", value)
            )
    ...
    def _on_energy_unit_change(self, value):
        ...
        if "kcal/mol" in current_ylabel:
            ...
            new_ylabel = (
                current_ylabel.replace("kJ/mol", value)
                .replace("kj/mol", value)
                .replace("KJ/mol", value)
            )
        else:
            # If no unit found, append it
    ...
    def _on_rmsf_format_change(self, value):
        ...
        """
        # Map display names to internal values
        mapping = {"1-Letter Code": "single", "3-Letter Code": "triple"}
    ...
    def _detect_property_type_from_column(self, column_display_name):
        ...
        if "temperature" in column_lower or "temp" in column_lower:
            ...
            "energy" in column_lower
            or "elect" in column_lower
            or "vdw" in column_lower
            or "bond" in column_lower
            or "angle" in column_lower
            or "dihed" in column_lower
            or "poteng" in column_lower
            or "kineng" in column_lower
            or "toteng" in column_lower
        ):
            return "energy"
        else:
    ...
    def _convert_units(self, data, data_key, property_type, target_unit):
        ...
        if property_type == "energy":
            ...
            if data_key.lower() in energy_column_keys:
                conversion = self.unit_systems["energy"]["conversions"].get(
                    target_unit, 1.0
                )
        ...
        # Pressure conversion (atm is native NAMD unit)
            ...
            if "pressure" in data_key.lower():
                conversion = self.unit_systems["pressure"]["conversions"].get(
                    target_unit, 1.0
                )
                return [val * conversion for val in data]
        ...
        # Volume conversion (Å³ is native NAMD unit)
            ...
            if "volume" in data_key.lower():
                conversion = self.unit_systems["volume"]["conversions"].get(
                    target_unit, 1.0
                )
                return [val * conversion for val in data]
    ...
    def _reset_plot_settings(self):
        ...
        # Common defaults
        self.plot_time_units.set(
            "ns"
        )  # Default to nanoseconds to match initial defaults
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        if data.shape[1] >= 2:  # Should have time and RMSD columns
                            loaded_data = {"time": data[:, 0], "rmsd": data[:, 1]}
                            break
                    except:
                        continue
    ...
    def _plot_rmsf_data(self, rmsf_data, residue_info):
        ...
        self.ax.plot(
            x_values, plot_rmsf, color=line_color, linewidth=2, marker="o", markersize=3
        )
        ...
        if x_labels and self.rmsf_show_residue_labels.get():
            ...
            if frequency_setting == "all":
                ...
                step = int(
                    frequency_setting.split("_")[1]
                )  # Extract number from "every_N"
                tick_positions = list(range(0, n_labels, step))
                tick_labels = [x_labels[i] for i in tick_positions]
    ...
    def _plot_distance_data(self):
        ...
        if (
            "distance" not in self.analysis_results
            or "time" not in self.analysis_results
        ):
            return
        ...
        if text_color_setting == "Auto":
            if bg_color == "none":
                ...
                try:
                    ...
                    text_color = (
                        "white" if bg_color not in ["#ffffff", "#f0f0f0"] else "black"
                    )
        elif text_color_setting == "Custom RGB":
            if hasattr(self, "custom_text_color_entry"):
                custom_color = self.custom_text_color_entry.get().strip()
    ...
    def create_file_section(self, row):
        ...
        self.namd_file_title_label.grid(
            row=0, column=0, columnspan=3, pady=(10, 5), sticky="w", padx=10
        )
        ...
        topology_btn = ctk.CTkButton(
            file_frame, text="Browse", command=self.select_topology_file, width=80
        )
        ...
        output_btn = ctk.CTkButton(
            file_frame, text="Browse", command=self.select_output_directory, width=80
        )
    ...
    def create_analysis_section(self, row):
        ...
        ref_frame_entry = ctk.CTkEntry(
            settings_frame, textvariable=self.reference_frame, width=100
        )
    ...
    def create_plot_section(self, row):
        ...
        plot_title = ctk.CTkLabel(
            plot_frame, text="Plot Settings", font=ctk.CTkFont(size=14, weight="bold")
        )
        plot_title.grid(
            row=0, column=0, columnspan=4, pady=(10, 5), sticky="w", padx=10
        )
        ...
        self.plot_checkbox.grid(
            row=1, column=0, columnspan=4, pady=5, padx=10, sticky="w"
        )
        ...
        plot_title_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_title, width=150
        )
        ...
        plot_xlabel_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlabel, width=150
        )
        ...
        plot_ylabel_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylabel, width=150
        )
        ...
        plot_xlim_min_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlim_min, width=80
        )
        ...
        plot_xlim_max_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_xlim_max, width=80
        )
        ...
        plot_ylim_min_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylim_min, width=80
        )
        ...
        plot_ylim_max_entry = ctk.CTkEntry(
            plot_frame, textvariable=self.plot_ylim_max, width=80
        )
        plot_ylim_max_entry.grid(
            row=6, column=3, pady=(5, 10), padx=(5, 10), sticky="w"
        )
    ...
    def create_results_section(self, row):
        ...
        self.results_textbox = ctk.CTkTextbox(
            results_frame, font=ctk.CTkFont(family="Consolas", size=10), wrap="word"
        )
        ...
        self.plot_main_frame.grid(
            row=row, column=1, sticky="nsew", padx=(5, 10), pady=5
        )
        ...
        self.plot_display_frame = ctk.CTkFrame(
            self.plot_main_frame, height=400, width=600
        )
        self.plot_display_frame.grid(
            row=1, column=0, sticky="nsew", padx=10, pady=(5, 10)
        )
    ...
    def create_control_section(self, row):
        ...
        clear_button = ctk.CTkButton(
            button_frame, text="Clear", command=self.clear_all, height=40, width=100
        )
        ...
        if self.get_current_pdb:
            ...
            if pdb_file and os.path.exists(pdb_file):
                ...
                messagebox.showwarning(
                    "No PDB", "No valid PDB file available in the current project."
                )
        else:
            messagebox.showwarning(
                "Not Available", "Current PDB loading not available."
            )
    ...
    def add_trajectory_file(self, file_type="dcd"):
        ...
        filenames = filedialog.askopenfilenames(
            title=title, filetypes=filetypes, initialdir=self.initial_directory
        )
        ...
        if filenames:
            ...
            if added_count > 0:
                ...
                self.status_callback(
                    f"{added_count} {file_type_name} {files_text} added"
                )
                ...
                if (
                    first_log_added
                    and hasattr(self, "namd_available_columns")
                    and self.namd_available_columns
                ):
                    self.status_callback(
                        f"Detected {len(self.namd_available_columns)} columns from log file"
                    )
            ...
            if duplicate_count > 0:
                ...
                messagebox.showwarning(
                    "Duplicate Files",
                    f"{duplicate_count} {files_text} already in the list.",
                )
    ...
    def _create_draggable_file_bar(self, row_idx, filepath):
        ...
        time_entry = ctk.CTkEntry(file_bar, textvariable=time_var, width=80, height=28)
        time_entry.grid(row=0, column=2, padx=5, pady=3)

        # Update time dict when entry changes
        time_entry.bind(
            "<FocusOut>",
            lambda e, fp=filepath, tv=time_var: self._update_file_time(fp, tv),
        )
        time_entry.bind(
            "<Return>",
            lambda e, fp=filepath, tv=time_var: self._update_file_time(fp, tv),
    ...
    def _reorder_file(self, filepath, rows_moved):
        ...
        try:
            ...
            if new_idx != current_idx:
                ...
                self.status_callback(
                    f"Moved {Path(filepath).name} to position {new_idx + 1}"
                )
    ...
    def _update_file_time(self, filepath, time_var):
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Time", "Please enter a valid number for simulation time."
            )
    ...
    def remove_selected_file(self):
        ...
        if (
            hasattr(self, "selected_file")
            and self.selected_file
            and self.selected_file in self.trajectory_files
        ):
            ...
            messagebox.showinfo(
                "No Selection",
                "Please click on a file to select it, then click Remove.",
            )
    ...
    def _auto_detect_file_times(self):
        ...
        try:
                ...
                if current_tab == "Structural":
                    ...
                    try:
                        ...
                        if not topology_file or not os.path.exists(topology_file):
                            raise ValueError(
                                "Valid topology file required for auto-detection"
                            )
                        ...
                        logger.warning(
                            f"Could not auto-detect time for {Path(filepath).name}: {e}"
                        )
                        # Use default estimate
                        self.file_times[filepath] = (
            ...
            messagebox.showinfo(
                ...
                "Simulation times have been estimated from file data.",
            )

        except Exception as e:
            messagebox.showerror(
                "Auto-Detection Failed", f"Could not auto-detect times:\n\n{str(e)}"
            )
    ...
    def _calculate_time_array_for_trajectories(self, universe):
        ...
        for traj_file in self.trajectory_files:
            ...
            if duration_ns > 0 and n_frames > 1:
                ...
                file_times = (
                    cumulative_time_ns + np.arange(n_frames) * 0.01
                )  # 10ps per frame in ns
    ...
    def add_trajectory_folder(self, file_type="dcd"):
        ...
        """
        title = (
            "Select Equilibration Folder"
            if file_type == "dcd"
            else "Select Folder with Log Files"
        )
        directory = filedialog.askdirectory(
            title=title, initialdir=self.initial_directory
        )
        ...
        if directory:
            ...
            if found_files:
                ...
                if added_count > 0:
                    ...
                    files_text = "file" if added_count == 1 else "files"
                    self.status_callback(
                        f"Added {added_count} {file_type_name} {files_text} from folder"
                    )
                ...
                if duplicate_count > 0:
                    files_text = "file" if duplicate_count == 1 else "files"
                    messagebox.showinfo(
                        "Info", f"{duplicate_count} {files_text} already in the list."
                    )
            else:
                file_ext = ".log" if file_type == "log" else ".dcd/trajectory"
                messagebox.showwarning(
                    "No Files Found",
                    f"No {file_ext} files found in the selected directory.",
                )
        ...
        for ext in topology_extensions:
            ...
            if topology_files:
                ...
                if not self.topology_file.get():  # Only set if not already set
                    ...
                    self.status_callback(
                        f"Auto-discovered topology: {topology_files[0].name}"
                    )
                break
    ...
    def select_output_directory(self):
        ...
        directory = filedialog.askdirectory(
            title="Select Output Directory", initialdir=self.initial_directory
        )
        if directory:
    ...
    def run_analysis(self):
        ...
        if analysis_type == "-":
            messagebox.showwarning(
                "No Analysis Type", "Please select an analysis type first."
            )
            return
    ...
    def run_log_analysis(self):
        ...
        if self.trajectory_files and not self.analysis_results:
            try:
                ...
                messagebox.showerror(
                    "Loading Failed", f"Failed to load log files:\n\n{str(e)}"
                )
                return
        ...
        if not self.analysis_results:
            messagebox.showerror(
                "No Data", "No data available. Please check your log files."
            )
            return

        # Ensure NAMD options UI is created (creates the combo box and checkboxes)
        if not hasattr(self, "namd_column_combo") or not hasattr(
            self, "column_checkboxes"
        ):
        ...
        if not selected_columns or (
            len(selected_columns) == 1 and selected_columns[0] == "No data available"
        ):
            messagebox.showwarning(
                "No Column Selected",
                "Please select which data to plot from the available columns.",
            )
            return
        ...
        self.status_callback(
            f"NAMD log analysis completed ({num_cols} {col_word} plotted)"
        )
    ...
    def _load_combined_namd_logs(self):
        ...
        self._file_timestep_ranges = (
            {}
        )  # Store (start_idx, end_idx, min_ts, max_ts) for each file
        ...
        print(
            f"Loading combined NAMD logs from {len(self.trajectory_files)} files in UI order"
        )
            ...
            try:
                ...
                if file_data:
                    ...
                    if num_points > 0 and "timestep" in file_data:
                        ...
                        self._file_timestep_ranges[log_path] = (
                            current_idx,
                ...
                self.status_callback(
                    f"Warning: Could not parse {Path(log_path).name}: {str(e)}"
                )

        print(
            f"Combined data summary: {[(key, len(values)) for key, values in combined_data.items()]}"
        )

        if combined_data["timestep"]:
            self.analysis_results = combined_data
            self.status_callback(
                f"Loaded combined NAMD log data ({len(combined_data['timestep'])} total data points)"
            )
        else:
            messagebox.showerror(
                "Error", "No data could be loaded from the discovered log files."
            )
    ...
    def run_trajectory_analysis(self):
        ...
        if not self.trajectory_files:
            messagebox.showerror(
                "Missing Input", "Please add at least one trajectory file."
            )
            return
        ...
        if self.running_analysis:
            messagebox.showwarning(
                "Analysis Running", "An analysis is already running."
            )
            return
        ...
        try:
            ...
            if len(self.trajectory_files) == 1:
                ...
                self._update_results(
                    f"Loading trajectory: {Path(self.trajectory_files[0]).name}...\n"
                )
                ...
                self._update_results(
                    f"Loading {len(self.trajectory_files)} trajectory files...\n"
                )
                universe = mda.Universe(topology_file, self.trajectory_files)  # type: ignore[possibly-unbound]
                total_frames = len(universe.trajectory)
                self._update_results(
                    f"Successfully loaded and concatenated {len(self.trajectory_files)} trajectories\n"
                )
            ...
            if selection_text == "Custom" and self.use_custom_selection.get():
                ...
                if custom_sel:
                    try:
                        ...
                        self._update_results(
                            f"Using custom selection: '{custom_sel}'\n"
                        )
                    except Exception as e:
            ...
            if analysis_type.upper() == "RMSD":
                ...
                self._update_results(
                    f"Analysis type '{analysis_type}' not implemented yet.\n"
                )
    ...
    def _run_rmsd_analysis(self, universe, atoms, selection_string):
        ...
        if self.align_rmsd.get():
            ...
            aligner.run()
            self._update_results(
                "Alignment completed (rotation + translation applied).\n"
            )
            ...
            self._update_results(
                "Calculating RMSD without alignment (raw coordinates)...\n"
            )
        ...
        if filename:
            try:
                ...
                with open(filename, "w", newline="") as f:
                    ...
                    # Write header
                    if (
                        "time" in self.analysis_results
                        and "rmsd" in self.analysis_results
                    ):
                        writer.writerow(["Time(ps)", "RMSD(nm)"])
                        # Write data
                        for time, rmsd in zip(
                            self.analysis_results["time"], self.analysis_results["rmsd"]
                        ):
    ...
    def export_json(self):
        ...
        if filename:
            try:
                ...
                messagebox.showerror(
                    "Export Error", f"Failed to export JSON:\n{str(e)}"
                )
    ...
    def export_numpy(self):
        ...
        if filename:
            try:
                ...
                messagebox.showerror(
                    "Export Error", f"Failed to export NumPy:\n{str(e)}"
                )
    ...
    def _run_rmsf_analysis(self, universe, atoms, selection_string):
        ...
        if ref_frame >= len(universe.trajectory):
            ref_frame = 0
            self._update_results(
                f"Reference frame {self.reference_frame.get()} out of range, using frame 0\n"
            )
        ...
        if selection_text not in ["name CA", "name CB"]:
            self._update_results(
                "WARNING: For residue-based plotting, select 'name CA' or 'name CB'.\n"
            )
            self._update_results(
                "         Current selection may have multiple atoms per residue.\n"
            )
        ...
        with open(output_file, "w") as f:
            f.write(
                f"# Atom_Index\tResidue_Number\tResidue_Name\tChain\tRMSF(Å)\tRMSF({unit_label})\n"
            )
            for i, rmsf in enumerate(rmsf_values):
                rmsf_display = rmsf_values_display[i]
                res_info = residue_info[i]
                f.write(
                    f"{res_info['atom_index']}\t{res_info['residue_number']}\t"
    ...
    def _run_distance_analysis(self, universe):
        ...
        if not selection1_text or not selection2_text:
            error_msg = (
                "ERROR: Both selections must be specified for distance analysis.\n"
            )
            self._update_results(error_msg)
        ...
        try:
            ...
            self._update_results(
                f"Selection 1 ('{selection1_text}'): {len(atoms1)} atoms\n"
            )
            self._update_results(
                f"Selection 2 ('{selection2_text}'): {len(atoms2)} atoms\n"
            )
    ...
    def _run_rg_analysis(self, universe, atoms):
        ...
        try:
            ...
            self._update_results(
                f"Selection ('{selection_text}'): {len(rg_atoms)} atoms\n"
            )
    ...
    def _export_to_numpy(self, filename):
        ...
        if "rmsd" in self.analysis_results:
            data_to_save = {
                ...
                "time": self.analysis_results.get(
                    "time", np.arange(len(self.analysis_results["rmsd"]))
                ),
            }
        else:
    ...
    def _bind_mouse_wheel(self, widget):
        ...
        def _on_mouse_wheel(event):
            ...
            while current:
                ...
                if hasattr(current, "master") and isinstance(
                    current.master, ctk.CTkTextbox
                ):
            ...
            if textbox_widget:
                try:
                    if hasattr(textbox_widget, "_textbox"):
                        textbox_widget._textbox.yview_scroll(
                            int(-1 * (event.delta / 120)), "units"
                        )
                except Exception:
                    pass
    ...
    def _on_analysis_type_change(self, value):
        ...
        if value == "RMSF":
            ...
            if (
                ...
                self._plot_rmsf_data(
                    self.analysis_results["rmsf"], self.analysis_results["residue_info"]
                )
            else:
            ...
            if "rmsd" in self.analysis_results and "time" in self.analysis_results:
                self._plot_rmsd_data(
                    self.analysis_results["rmsd"], self.analysis_results["time"]
                )
            else:
        ...
        if hasattr(self, "run_button"):
            ...
            if current_tab == "Energetic":
                ...
                self.run_button.configure(
                    text="Run Trajectory Analysis", command=self.run_analysis
                )
            else:
                # Default state
                self.run_button.grid()
                self.run_button.configure(
                    text="Select Analysis Type", command=None, state="disabled"
                )
    ...
    def _show_trajectory_analysis_options(self):
        ...
        if analysis_type == "Distances":
            ...
            sel1_entry.grid(
                row=row, column=1, columnspan=2, pady=5, padx=(5, 10), sticky="ew"
            )
            ...
            analyze_btn.grid(
                row=row, column=0, columnspan=3, pady=5, padx=(10, 10), sticky="w"
            )
            ...
            help_text.grid(
                row=row, column=0, columnspan=3, pady=(0, 5), padx=(10, 10), sticky="w"
            )
            ...
            # Atom selection
            atom_label = ctk.CTkLabel(
                self.dynamic_options_frame, text="Atom Selection:"
            )
            ...
            atom_combo = ctk.CTkComboBox(
                ...
                values=[
                    ...
                    "all",
                    "Custom",
                ],  # Protein and name C is ideal for RMSF analysis as it also considers the ACE and NME caps.
                variable=self.atom_selection,
                width=150,
            ...
            self.custom_selection_label = ctk.CTkLabel(
                self.dynamic_options_frame, text="Custom Selection:"
            )
            ...
            if self.atom_selection.get() == "Custom":
                self.custom_selection_label.grid(
                    row=row, column=0, pady=5, padx=(10, 5), sticky="w"
                )
                self.custom_selection_entry.grid(
                    row=row, column=1, columnspan=2, pady=5, padx=(5, 10), sticky="ew"
                )
            ...
            if analysis_type in ["RMSD", "RMSF"]:
                ref_label = ctk.CTkLabel(
                    self.dynamic_options_frame, text="Reference Frame:"
                )
    ...
    def _on_atom_selection_change(self, value):
        ...
        if value == "Custom":
            ...
            self.dynamic_widgets.extend(
                [self.custom_selection_label, self.custom_selection_entry]
            )
...
"""
        ...
        close_btn = ctk.CTkButton(
            help_window, text="Close", command=help_window.destroy, width=100
        )
    ...
    def _analyze_topology(self):
        ...
        if not MDANALYSIS_AVAILABLE:
            messagebox.showerror(
                "MDAnalysis Not Available",
                "MDAnalysis is required for topology analysis.",
            )
            return
        ...
        try:
            ...
            self._topology_font_size = (
                36  # Base font size (increased to 36pt for much better visibility)
            )
            ...
            analysis_window.bind(
                "<Control-equal>", safe_font_increase
            )  # = key (often same as +)
            analysis_window.bind("<Control-minus>", safe_font_decrease)
            analysis_window.bind(
                "<Control-KP_Subtract>", safe_font_decrease
            )  # Numpad -
            ...
            for residue in universe.residues:  # type: ignore[union-attr]
                ...
                if residue.resname.upper() in protein_residues:
                    ...
                    residue.resname.upper() in lipid_names
                    or "LIP" in residue.resname.upper()
                ):
            ...
            for category, residues in residue_categories.items():
                if residues:
                    summary_content += f"\n{'='*80}\n"
                    summary_content += (
                        f"{category.upper()} ({len(residues)} residues)\n"
                    )
                    summary_content += f"{'='*80}\n\n"

                    # Category header with icon
                    cat_frame = ctk.CTkFrame(
                        summary_scroll, fg_color=("gray85", "gray20")
                    )
                    cat_frame.grid(
                        row=row_idx, column=0, sticky="ew", padx=5, pady=(10, 5)
                    )
                    ...
                    content_frame = ctk.CTkFrame(
                        summary_scroll, fg_color=("gray95", "gray15")
                    )
                    content_frame.grid(
                        row=row_idx, column=0, sticky="ew", padx=5, pady=(0, 5)
                    )
                    row_idx += 1
                    ...
                    for resname in sorted(by_name.keys()):
                        ...
                        res_name_frame = ctk.CTkFrame(
                            content_frame, fg_color="transparent"
                        )
                        res_name_frame.grid(
                            row=inner_row, column=0, sticky="ew", padx=10, pady=5
                        )
                        ...
                        if len(res_list) <= 20:
                            ...
                            # First 5
                            first_5 = "First 5: " + ", ".join(
                                [f"({r['index']},{r['resSeq']})" for r in res_list[:5]]
                            )
                            ...
                            first_label.grid(
                                row=inner_row, column=0, sticky="w", padx=20, pady=1
                            )
                            inner_row += 1
            ...
            detailed_textbox = ctk.CTkTextbox(
                tab_detailed, wrap="word", font=self._topology_fonts["text"]
            )
            ...
            examples_scroll = ctk.CTkScrollableFrame(
                tab_examples, fg_color="transparent"
            )
            ...
            def create_selection_button(parent, description, selection, row):
                ...
                copy_btn = ctk.CTkButton(
                    ...
                    width=80,
                    height=28,
                    command=lambda s=selection: self._copy_to_clipboard(
                        s, analysis_window
                    ),
                )
            ...
            if residue_categories["Protein"]:
                ...
                protein_header.grid(
                    row=row_counter, column=0, sticky="w", padx=5, pady=(10, 5)
                )
                row_counter += 1
                ...
                create_selection_button(
                    examples_scroll, "All protein atoms", "protein", row_counter
                )
                row_counter += 1
                ...
                # By PDB numbering
                sel_resseq = (
                    f"protein and resSeq {min_seq} to {min(min_seq+50, max_seq)}"
                )
                create_selection_button(
                    examples_scroll,
            ...
            if residue_categories["Lipids"]:
                ...
                lipid_header.grid(
                    row=row_counter, column=0, sticky="w", padx=5, pady=(15, 5)
                )
                row_counter += 1
                ...
                create_selection_button(
                    examples_scroll, "All lipids", sel_all_lipids, row_counter
                )
                row_counter += 1
            ...
            if residue_categories["Water"]:
                ...
                # All water
                create_selection_button(
                    examples_scroll, "All water molecules", "water", row_counter
                )
                row_counter += 1
            ...
            if residue_categories["Ions"]:
                ...
                ion_info_label.grid(
                    row=row_counter, column=0, sticky="w", padx=10, pady=3
                )
                row_counter += 1
                ...
                create_selection_button(
                    examples_scroll, "All ions", sel_all_ions, row_counter
                )
                row_counter += 1
            ...
            if residue_categories["Other"]:
                ...
                for resname in sorted(other_names_found):
                    create_selection_button(
                        examples_scroll, resname, f"resname {resname}", row_counter
                    )
                    row_counter += 1
            ...
            if residue_categories["Protein"] and residue_categories["Lipids"]:
                lipid_names_found = set(r["name"] for r in residue_categories["Lipids"])
                sel_prot_lip = (
                    f"protein or resname {' '.join(sorted(lipid_names_found))}"
                )
                create_selection_button(
                    examples_scroll,
                    "Protein and lipids together",
                    sel_prot_lip,
                    row_counter,
                )
                row_counter += 1
            ...
            create_selection_button(
                examples_scroll, "Everything except water", "not water", row_counter
            )
            row_counter += 1
...
* Click the Copy button to copy any selection to clipboard"""
            ...
            export_btn = ctk.CTkButton(
                ...
                command=lambda: self._export_topology_analysis(
                    summary_content, detailed_content, examples_content
                ),
                width=200,
                height=35,
                font=self._topology_fonts["button"],
            ...
            messagebox.showerror(
                "Analysis Error", f"Failed to analyze topology:\n{str(e)}"
            )
    ...
    def _store_original_fonts(self, window):
        ...
        def store_fonts(widget):
            try:
                ...
                if any(
                    t in widget_type for t in ["CTkLabel", "CTkButton", "CTkTextbox"]
                ):
                    try:
                        # Try _font attribute first
                        font_attr = getattr(widget, "_font", None)
                        if font_attr and hasattr(font_attr, "cget"):
                            self._topology_original_fonts[widget_id] = font_attr.cget(
                                "size"
                            )
                        else:
                            ...
                            if current_font and isinstance(current_font, ctk.CTkFont):
                                self._topology_original_fonts[widget_id] = (
                                    current_font.cget("size")
                                )
                    except:
                        pass
            ...
            if hasattr(self, "_topology_font_objects") and self._topology_font_objects:
                for font_obj, original_size in self._topology_font_objects:
                    try:
                        ...
                        min_size = max(
                            6, int(original_size * 0.5)
                        )  # At least 50% of original or 6pt
                ...
                self.status_callback(
                    f"Font size: {self._topology_font_size}pt (no fonts to update)"
                )
        ...
        if filename:
            try:
                with open(filename, "w") as f:
                    ...
                    f.write(examples)
                messagebox.showinfo(
                    "Success", f"Topology analysis exported to:\n{filename}"
                )
            except Exception as e:
                messagebox.showerror(
                    "Export Error", f"Failed to export analysis:\n{str(e)}"
                )
    ...
    def update_fonts(self, scaled_fonts):
        ...
        try:
            ...
            combo_widgets = [
                "analysis_type_combo",
                "plot_time_units_combo",
                "rmsf_xaxis_combo",
                "rmsf_format_combo",
                "rmsf_frequency_combo",
                "plot_yaxis_units_combo",
                "namd_column_combo",
            ]
            ...
            input_widgets = [
                "custom_line_color_entry",
                "custom_bg_color_entry",
                "custom_fig_bg_color_entry",
                "custom_text_color_entry",
                "custom_selection_entry",
                "topology_file_entry",
    ...
    def _mark_analysis_ready(self):
        ...
        if hasattr(self, "status_button"):
            try:
                ...
                self.status_button.configure(
                    text="Ready", fg_color="#27A85D", hover_color="#1B7440"
                )
            except (AttributeError, tk.TclError):
                # Widget doesn't exist or has been destroyed, ignore
                pass
                    ...
                    if line.startswith("ETITLE:"):
                        ...
                        for header in headers:
                            if header not in ["TS", "timestep"]:
                                display_name = column_mapping.get(
                                    header.upper(), f"{header.title()}"
                                )
                                self.namd_available_columns.append(display_name)

                        logger.info(
                            f"Detected {len(self.namd_available_columns)} NAMD columns: {self.namd_available_columns}"
                        )
                        return
        except Exception as e:
    ...
    def _show_namd_log_options(self):
        ...
        multi_label = ctk.CTkLabel(
            self.dynamic_options_frame, text="Add Multiple Columns:"
        )
        ...
        if (
            self.analysis_results
            and namd_columns
            and namd_columns[0] != "No data available"
        ):
    ...
    def _get_available_namd_columns(self):
        ...
        if hasattr(self, "analysis_results") and self.analysis_results:
            ...
            for key in self.analysis_results.keys():
                ...
                if (
                    key not in ["timestep", "TS"]
                    and key in self.analysis_results
                    and self.analysis_results.get(key) is not None
                ):
                    # Also check if it's a list/array with content
                    value = self.analysis_results[key]
                    if (isinstance(value, (list, np.ndarray)) and len(value) > 0) or (
                        not isinstance(value, (list, np.ndarray))
                    ):
                        display_name = column_mapping.get(
                            key.upper(), f"{key.title()} (unit)"
                        )
    ...
    def _on_multi_column_change(self):
        ...
        if self.plot_enabled.get():
            ...
            try:
                ...
                logger.error(
                    f"Error updating NAMD plot from multi-column change: {e}",
                    exc_info=True,
                )
        ...
        if hasattr(self, "trajectory_listbox"):
            ...
            for widget in self.trajectory_listbox.master.winfo_children():
                if isinstance(widget, ctk.CTkButton) and (
                    widget.cget("text") in ["Add", "Remove"]
                ):
                    widget.grid_remove()
        ...
        if not hasattr(self, "namd_log_button"):
            ...
            for child in self.left_panel.winfo_children():
                if isinstance(child, ctk.CTkFrame):
                    for subchild in child.winfo_children():
                        if (
                            isinstance(subchild, ctk.CTkLabel)
                            and subchild.cget("text") == "Input Files"
                        ):
                            file_frame = child
                            break
                    if file_frame:
            ...
            if file_frame:
                ...
                self.namd_log_button.grid(
                    row=2, column=1, columnspan=2, pady=5, padx=5, sticky="ew"
                )
        else:
            self.namd_log_button.grid()
    ...
    def _update_plot_labels_for_analysis_type(self, analysis_type):
        ...
        label_mapping = {
            ...
            "RMSD": {
                ...
                "ylabel": f"RMSD ({distance_unit})",
            },
            "RMSF": {
                "title": "RMSF Analysis",
                "xlabel": "Residue",
                "ylabel": f"RMSF ({distance_unit})",
            },
            "Distances": {
                "title": "Distance Analysis",
                "xlabel": f"Time ({time_unit})",
                "ylabel": f"Distance ({distance_unit})",
            },
            "Radius of Gyration": {
                "title": "Radius of Gyration",
                "xlabel": f"Time ({time_unit})",
                "ylabel": f"Rg ({distance_unit})",
            },
            "NAMD Log Analysis": {
                "title": "NAMD Log Analysis",
                "xlabel": f"Time ({time_unit})",
                "ylabel": "Value",
            },
        }
    ...
    def parse_namd_log(self, log_file_path):
        ...
        try:
                    ...
                    if len(parts) >= len(column_mapping):
                        try:
                            ...
                            if timestep in timesteps_seen:
                                duplicates_found += 1
                                print(
                                    f"Warning: Duplicate timestep {timestep} found, skipping (duplicate #{duplicates_found})"
                                )
                                continue
                            ...
                            if (
                                ...
                                print(
                                    f"Warning: Non-monotonic timestep {timestep} after {previous_timestep}, keeping but noting"
                                )
                            ...
                            for header, index in column_mapping.items():
                                if header != "TS" and index < len(
                                    parts
                                ):  # Skip TS as it's already processed
                                    try:
                                        value = float(parts[index])
                                        # Check for unrealistic values that might indicate parsing errors
                                        if (
                                            abs(value) > 1e10
                                        ):  # Very large values might be parsing errors
                                            print(
                                                f"Warning: Unusually large value {value} for {header} at timestep {timestep}"
                                            )
            ...
            if data["timestep"]:
                ...
                print(
                    f"Timestep range: {min(data['timestep'])} to {max(data['timestep'])}"
                )
    ...
    def _validate_namd_data_quality(self, data, duplicates_found):
        ...
        if len(timesteps) > 1:
            timestep_diffs = [
                timesteps[i + 1] - timesteps[i] for i in range(len(timesteps) - 1)
            ]
            most_common_diff = max(set(timestep_diffs), key=timestep_diffs.count)

            large_gaps = [
                i
                for i, diff in enumerate(timestep_diffs)
                if diff > most_common_diff * 2
            ]
            if large_gaps:
                print(
                    f"Warning: Found {len(large_gaps)} large timestep gaps at indices: {large_gaps[:5]}"
                )
        ...
        for key, values in data.items():
            if key != "timestep":
                ...
                if none_count > 0:
                    ...
                    print(
                        f"Warning: {key} has {none_count} missing values ({percentage:.1f}%)"
                    )

        if duplicates_found > 0:
            print(
                f"Data quality summary: {duplicates_found} duplicate timesteps removed"
            )
    ...
    def select_namd_log_file(self):
        ...
        if filename:
            ...
            if data and data["timestep"]:
                ...
                self.status_callback(
                    f"NAMD log loaded: {Path(filename).name} ({len(data['timestep'])} data points)"
                )
                ...
                messagebox.showerror(
                    "Error", "Could not parse NAMD log file or no data found."
                )
    ...
    def _auto_load_log_files(self):
            ...
            try:
                ...
                if data and data["timestep"]:
                    ...
                    if num_points > 0:
                        ...
                        min_ts = min(timesteps)
                        max_ts = max(timesteps)
                        self._file_timestep_ranges[log_file_path] = (
                            current_idx,
                            current_idx + num_points,
                            min_ts,
                            max_ts,
                        ...
                        print(
                            f"File {Path(log_file_path).name}: indices {current_idx}-{current_idx + num_points}, timesteps {min_ts}-{max_ts}"
                        )
                        current_idx += num_points
        ...
        if total_points > 0:
            ...
            print(
                f"Populated namd_available_columns with {len(self.namd_available_columns)} columns: {self.namd_available_columns}"
            )

            num_files = len(self.trajectory_files)
            file_word = "file" if num_files == 1 else "files"
            self.status_callback(
                f"Loaded {num_files} log {file_word} ({total_points} data points)"
            )

            print(
                f"Created _file_timestep_ranges with {len(self._file_timestep_ranges)} files"
            )
    ...
    def get_selected_namd_data(self):
        ...
        if (
            data_key
            and data_key in self.analysis_results
            and "timestep" in self.analysis_results
        ):
    ...
    def _calculate_time_for_namd_logs(self):
        ...
        print(
            f"Assigned times: {[(Path(f).name, t) for f, t in self.file_times.items()]}"
        )
        ...
        for filepath in self.trajectory_files:
            ...
            print(
                f"  Assigned time: {assigned_time_ps}ps ({assigned_time_ps/1000.0}ns)"
            )
            ...
            if assigned_time_ps <= 0:
                ...
                if num_points == 1:
                    ...
                    print(
                        f"  Distributed {num_points} points from {cumulative_time_ps}ps to {cumulative_time_ps + assigned_time_ps}ps"
                    )
        ...
        print(
            f"Time range: {min(time_array_ps) if time_array_ps else 'N/A'} to {max(time_array_ps) if time_array_ps else 'N/A'} ps"
        )
    ...
    def _plot_namd_data(self):
        ...
        if not self.trajectory_files:
            messagebox.showerror(
                "No Files", "No log files added. Please add log files before plotting."
            )
            return
        ...
        if not self.analysis_results or "timestep" not in self.analysis_results:
            messagebox.showerror(
                "No Data",
                "Failed to load data from log files. Please check that the files are valid NAMD log files.",
            )
            return
        ...
        if selected_columns == ["No data available"]:
            messagebox.showwarning(
                "No Data",
                "No data available for plotting. Please check your log files.",
            )
            return
        ...
        line_color = (
            self.plot_line_color.get() if hasattr(self, "plot_line_color") else "blue"
        )
        if line_color == "Custom RGB":
            # Use custom RGB value from entry
            if hasattr(self, "custom_line_color_entry"):
        ...
        has_assigned_times = bool(self.file_times) and any(
            time > 0 for time in self.file_times.values()
        )
        ...
        if len(x_data) != len(timesteps):
            print(
                f"ERROR: x_data length ({len(x_data)}) != y_data length ({len(timesteps)})"
            )
            messagebox.showerror(
                "Data Mismatch",
                f"Time array has {len(x_data)} points but data has {len(timesteps)} points. Cannot plot.",
            )
            return
        ...
        for i, column_display_name in enumerate(selected_columns):
            ...
            if data_key and data_key in self.analysis_results:
                ...
                if y_data:  # Check if data is not empty
                    ...
                    y_data = self._convert_units(
                        y_data, data_key, property_type, current_yaxis_unit
                    )
                    ...
                    line = self.ax.plot(
                        ...
                        marker="o",
                        markersize=2,
                        label=column_display_name.split("(")[0].strip(),
                    )
                    lines.extend(line)
        ...
        title = (
            self.plot_title.get()
            or f"NAMD Analysis - {len(selected_columns)} Properties"
        )
